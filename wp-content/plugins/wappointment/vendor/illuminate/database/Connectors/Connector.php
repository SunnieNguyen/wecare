...
class Connector
{
    ...
    {
        list($username, $password) = [$config['username'] ?? null, $config['password'] ?? null];
        try {
    ...
    {
        if (\class_exists(\WappoVendor\Doctrine\DBAL\Driver\PDOConnection::class) && !$this->isPersistentConnection($options)) {
            return new \WappoVendor\Doctrine\DBAL\Driver\PDOConnection($dsn, $username, $password, $options);
        }
