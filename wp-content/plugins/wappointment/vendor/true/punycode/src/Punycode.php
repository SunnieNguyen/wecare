...
class Punycode
{
    ...
    public function encode($input)
    {
        ...
        foreach ($parts as &$part) {
            $length = \strlen($part);
            if ($length < 1) {
                throw new \WappoVendor\TrueBV\Exception\LabelOutOfBoundsException(\sprintf('The length of any one label is limited to between 1 and 63 octets, but %s given.', $length));
            }
        ...
        if ($length > 255) {
            throw new \WappoVendor\TrueBV\Exception\DomainOutOfBoundsException(\sprintf('A full domain name is limited to 255 octets (including the separators), %s given.', $length));
        }
        return $output;
    }
    /**
