...
class MultipleValidationWithAnd implements \WappoVendor\Egulias\EmailValidator\Validation\EmailValidation
{
    /**
    ...
    public function __construct(array $validations, $mode = self::ALLOW_ALL_ERRORS)
    {
        if (\count($validations) == 0) {
            throw new \WappoVendor\Egulias\EmailValidator\Validation\Exception\EmptyValidationList();
        }
    ...
    public function isValid($email, \WappoVendor\Egulias\EmailValidator\EmailLexer $emailLexer)
    {
        ...
        if (!empty($errors)) {
            $this->error = new \WappoVendor\Egulias\EmailValidator\Validation\MultipleErrors($errors);
        }
        return $result;
    }
