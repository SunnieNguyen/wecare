...
trait PHPUnitAssertions
{
    ...
    public function assertVObjectEqualsVObject($expected, $actual, $message = '')
    {
        $getObj = function ($input) {
            ...
            if (\is_string($input)) {
                $input = \WappoVendor\Sabre\VObject\Reader::read($input);
            }
            if (!$input instanceof \WappoVendor\Sabre\VObject\Component) {
                $this->fail('Input must be a string, stream or VObject component');
            }
            unset($input->PRODID);
            if ($input instanceof \WappoVendor\Sabre\VObject\Component\VCalendar && 'GREGORIAN' === (string) $input->CALSCALE) {
