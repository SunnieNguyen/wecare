 ...
 */
class Component extends \WappoVendor\Sabre\VObject\Node
{
    /**
     * Component name.
     *
     ...
     * @param bool     $defaults
     */
    public function __construct(\WappoVendor\Sabre\VObject\Document $root, $name, array $children = [], $defaults = true)
    {
        ...
        if ($defaults) {
            ...
            foreach ($children as $key => $value) {
                if ($value instanceof \WappoVendor\Sabre\VObject\Node) {
            ...
            foreach ($children as $k => $child) {
                if ($child instanceof \WappoVendor\Sabre\VObject\Node) {
    ...
    public function add()
    {
        ...
        if ($arguments[0] instanceof \WappoVendor\Sabre\VObject\Node) {
    ...
    public function select($name)
    {
        ...
        if (!\is_null($name)) {
            ...
            if (\is_null($group)) {
                ...
                return \array_filter($result, function ($child) use($group) {
                    return $child instanceof \WappoVendor\Sabre\VObject\Property && \strtoupper($child->group) === $group;
                });
            }
        }
        ...
        foreach ($this->children as $childGroup) {
            foreach ($childGroup as $child) {
                if ($child instanceof \WappoVendor\Sabre\VObject\Property && \strtoupper($child->group) === $group) {
                    $result[] = $child;
                }
            }
        }
    ...
    public function __get($name)
    {
        ...
        if (0 === \count($matches)) {
            ...
            $firstMatch->setIterator(new \WappoVendor\Sabre\VObject\ElementList(\array_values($matches)));
            return $firstMatch;
        }
    }
    /**
    ...
    public function __set($name, $value)
    {
        $name = \strtoupper($name);
        $this->remove($name);
        if ($value instanceof self || $value instanceof \WappoVendor\Sabre\VObject\Property) {
            $this->add($value);
        } else {
            $this->add($name, $value);
        }
