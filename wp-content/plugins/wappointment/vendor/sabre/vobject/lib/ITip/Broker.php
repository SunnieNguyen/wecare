...
class Broker
{
     ...
     */
    public function processMessage(\WappoVendor\Sabre\VObject\ITip\Message $itipMessage, \WappoVendor\Sabre\VObject\Component\VCalendar $existingObject = null)
    {
    ...
    public function parseEvent($calendar = null, $userHref, $oldCalendar = null)
    {
        if ($oldCalendar) {
            if (\is_string($oldCalendar)) {
                $oldCalendar = \WappoVendor\Sabre\VObject\Reader::read($oldCalendar);
            }
        ...
        if (!\is_null($calendar)) {
            if (\is_string($calendar)) {
                $calendar = \WappoVendor\Sabre\VObject\Reader::read($calendar);
            }
     ...
     * @return VCalendar|null
     */
    protected function processMessageRequest(\WappoVendor\Sabre\VObject\ITip\Message $itipMessage, \WappoVendor\Sabre\VObject\Component\VCalendar $existingObject = null)
    {
        if (!$existingObject) {
            // This is a new invite, and we're just going to copy over
            // all the components from the invite.
            $existingObject = new \WappoVendor\Sabre\VObject\Component\VCalendar();
     ...
     * @return VCalendar|null
     */
    protected function processMessageCancel(\WappoVendor\Sabre\VObject\ITip\Message $itipMessage, \WappoVendor\Sabre\VObject\Component\VCalendar $existingObject = null)
    {
        if (!$existingObject) {
     ...
     * @return VCalendar|null
     */
    protected function processMessageReply(\WappoVendor\Sabre\VObject\ITip\Message $itipMessage, \WappoVendor\Sabre\VObject\Component\VCalendar $existingObject = null)
    {
        ...
        foreach ($instances as $recurId => $partstat) {
            $recurrenceIterator = new \WappoVendor\Sabre\VObject\Recur\EventIterator($existingObject, $itipMessage->uid);
            $found = false;
            $iterations = 1000;
            do {
     ...
     * @return array
     */
    protected function parseEventForOrganizer(\WappoVendor\Sabre\VObject\Component\VCalendar $calendar, array $eventInfo, array $oldEventInfo)
    {
        ...
        foreach ($attendees as $attendee) {
            ...
            $message = new \WappoVendor\Sabre\VObject\ITip\Message();
            ...
            $icalMsg = new \WappoVendor\Sabre\VObject\Component\VCalendar();
            ...
            if (!$attendee['newInstances']) {
                ...
                $event = $icalMsg->add('VEVENT', ['UID' => $message->uid, 'SEQUENCE' => $message->sequence, 'DTSTAMP' => \gmdate('WappoVendor\\Ymd\\THis\\Z')]);
                ...
                $message->significantChange = 'REQUEST' === $attendee['forceSend'] || \array_keys($attendee['oldInstances']) != \array_keys($attendee['newInstances']) || $oldEventInfo['significantChangeHash'] !== $eventInfo['significantChangeHash'];
                foreach ($attendee['newInstances'] as $instanceId => $instanceInfo) {
                    ...
                    $currentEvent->DTSTAMP = \gmdate('WappoVendor\\Ymd\\THis\\Z');
     ...
     * @return Message[]
     */
    protected function parseEventForAttendee(\WappoVendor\Sabre\VObject\Component\VCalendar $calendar, array $eventInfo, array $oldEventInfo, $attendee)
    {
        ...
        foreach ($instances as $instance) {
            ...
            if (isset($eventInfo['instances'][$instance['id']])) {
                ...
                // EXDATE
                $dt = \WappoVendor\Sabre\VObject\DateTimeParser::parse($instance['id'], $eventInfo['timezone']);
     ...
     * @return array
     */
    protected function parseEventInfo(\WappoVendor\Sabre\VObject\Component\VCalendar $calendar = null)
    {
        $uid = null;
        $organizer = null;
        $organizerName = null;
        ...
        foreach ($calendar->VEVENT as $vevent) {
            ...
            if (\is_null($uid)) {
                ...
                if ($uid !== $vevent->UID->getValue()) {
                    throw new \WappoVendor\Sabre\VObject\ITip\ITipException('If a calendar contained more than one event, they must have the same UID.');
                }
            }
            if (!isset($vevent->DTSTART)) {
                throw new \WappoVendor\Sabre\VObject\ITip\ITipException('An event MUST have a DTSTART property.');
            }
            if (isset($vevent->ORGANIZER)) {
                if (\is_null($organizer)) {
                    ...
                    if (\strtoupper($organizer) !== \strtoupper($vevent->ORGANIZER->getNormalizedValue())) {
                        throw new \WappoVendor\Sabre\VObject\ITip\SameOrganizerForAllComponentsException('Every instance of the event must have the same organizer.');
                    }
                }
