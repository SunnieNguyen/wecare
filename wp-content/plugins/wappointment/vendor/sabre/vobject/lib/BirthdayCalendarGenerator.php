...
class BirthdayCalendarGenerator
{
    ...
    public function setObjects($objects)
    {
        ...
        foreach ($objects as $object) {
            if (\is_string($object)) {
                $vObj = \WappoVendor\Sabre\VObject\Reader::read($object);
                if (!$vObj instanceof \WappoVendor\Sabre\VObject\Component\VCard) {
                    throw new \InvalidArgumentException('String could not be parsed as \\Sabre\\VObject\\Component\\VCard by setObjects');
                }
                $this->objects[] = $vObj;
            } elseif ($object instanceof \WappoVendor\Sabre\VObject\Component\VCard) {
                $this->objects[] = $object;
            } else {
    ...
    public function getResult()
    {
        ...
        foreach ($this->objects as $object) {
            ...
            $object = $object->convert(\WappoVendor\Sabre\VObject\Document::VCARD40);
            // Skip if the card has no FN property.
            if (!isset($object->FN)) {
                continue;
            }
            // Skip if the BDAY property is not of the right type.
            if (!$object->BDAY instanceof \WappoVendor\Sabre\VObject\Property\VCard\DateAndOrTime) {
                continue;
            }
            // Skip if we can't parse the BDAY value.
            try {
                $dateParts = \WappoVendor\Sabre\VObject\DateTimeParser::parseVCardDateTime($object->BDAY->getValue());
