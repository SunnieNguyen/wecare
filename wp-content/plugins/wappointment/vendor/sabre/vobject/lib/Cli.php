...
class Cli
{
    ...
    public function main(array $argv)
    {
        ...
        try {
            ...
            $this->log($this->colorize('green', 'sabre/vobject ') . $this->colorize('yellow', \WappoVendor\Sabre\VObject\Version::VERSION));
        ...
        } catch (\WappoVendor\Sabre\VObject\EofException $e) {
            // end of file
        } catch (\Exception $e) {
            $this->log('Error: ' . $e->getMessage(), 'red');
            return 2;
...
HELP
     ...
     * @return int
     */
    protected function validate(\WappoVendor\Sabre\VObject\Component $vObj)
    {
        $returnCode = 0;
        switch ($vObj->name) {
            case 'VCALENDAR':
        ...
        if (!\count($warnings)) {
            ...
            foreach ($warnings as $warn) {
                $extra = '';
                if ($warn['node'] instanceof \WappoVendor\Sabre\VObject\Property) {
     ...
     * @return int
     */
    protected function repair(\WappoVendor\Sabre\VObject\Component $vObj)
    {
        $returnCode = 0;
        switch ($vObj->name) {
            case 'VCALENDAR':
        ...
        $warnings = $vObj->validate(\WappoVendor\Sabre\VObject\Node::REPAIR);
        if (!\count($warnings)) {
            $this->log('  No warnings!');
        } else {
            $levels = [1 => 'REPAIRED', 2 => 'WARNING', 3 => 'ERROR'];
    ...
    protected function convert($vObj)
    {
        ...
        switch ($this->format) {
            case 'json':
                $json = true;
                if ('VCARD' === $vObj->name) {
                    $convertVersion = \WappoVendor\Sabre\VObject\Document::VCARD40;
                }
                break;
            case 'jcard':
                $json = true;
            ...
            case 'vcard':
                break;
            case 'vcard21':
                $convertVersion = \WappoVendor\Sabre\VObject\Document::VCARD21;
                break;
            case 'vcard30':
                $convertVersion = \WappoVendor\Sabre\VObject\Document::VCARD30;
                break;
            case 'vcard40':
                $convertVersion = \WappoVendor\Sabre\VObject\Document::VCARD40;
                break;
    ...
    protected function serializeComponent(\WappoVendor\Sabre\VObject\Component $vObj)
    {
        ...
        $sortScore = function ($key, $array) {
            if ($array[$key] instanceof \WappoVendor\Sabre\VObject\Component) {
                ...
                if ($array[$key] instanceof \WappoVendor\Sabre\VObject\Property) {
     ...
     */
    protected function serializeProperty(\WappoVendor\Sabre\VObject\Property $property)
    {
        if ($property->group) {
    ...
    protected function readInput()
    {
        if (!$this->parser) {
            ...
            if ('mimedir' === $this->inputFormat) {
                $this->parser = new \WappoVendor\Sabre\VObject\Parser\MimeDir($this->stdin, $this->forgiving ? \WappoVendor\Sabre\VObject\Reader::OPTION_FORGIVING : 0);
            } else {
                $this->parser = new \WappoVendor\Sabre\VObject\Parser\Json($this->stdin, $this->forgiving ? \WappoVendor\Sabre\VObject\Reader::OPTION_FORGIVING : 0);
            }
        }
