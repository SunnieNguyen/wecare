...
class RRuleIterator implements \Iterator
{
    ...
    {
        if (\is_string($rrule)) {
            $rrule = \WappoVendor\Sabre\VObject\Property\ICalendar\Recur::stringToArray($rrule);
        }
            ...
            switch ($key) {
                case 'FREQ':
                    ...
                    if (!\in_array($value, ['secondly', 'minutely', 'hourly', 'daily', 'weekly', 'monthly', 'yearly'])) {
                        throw new \WappoVendor\Sabre\VObject\InvalidDataException('Unknown value for FREQ=' . \strtoupper($value));
                    }
                    $this->frequency = $value;
                    break;
                case 'UNTIL':
                    $this->until = \WappoVendor\Sabre\VObject\DateTimeParser::parse($value, $this->startDate->getTimezone());
                ...
                case 'COUNT':
                    $val = (int) $value;
                    if ($val < 1) {
                        throw new \WappoVendor\Sabre\VObject\InvalidDataException(\strtoupper($key) . ' in RRULE must be a positive integer!');
                    }
                    $key = \strtolower($key);
                    $this->{$key} = $val;
                    break;
                ...
                case 'BYDAY':
                    ...
                    foreach ($value as $part) {
                        if (!\preg_match('#^  (-|\\+)? ([1-5])? (MO|TU|WE|TH|FR|SA|SU) $# xi', $part)) {
                            throw new \WappoVendor\Sabre\VObject\InvalidDataException('Invalid part in BYDAY clause: ' . $part);
                        }
                    }
                    $this->byDay = $value;
                    break;
                ...
                case 'BYYEARDAY':
                    ...
                    foreach ($this->byYearDay as $byYearDay) {
                        if (!\is_numeric($byYearDay) || (int) $byYearDay < -366 || 0 == (int) $byYearDay || (int) $byYearDay > 366) {
                            throw new \WappoVendor\Sabre\VObject\InvalidDataException('BYYEARDAY in RRULE must have value(s) from 1 to 366, or -366 to -1!');
                        }
                    }
                    break;
                case 'BYWEEKNO':
                    ...
                    foreach ($this->byWeekNo as $byWeekNo) {
                        if (!\is_numeric($byWeekNo) || (int) $byWeekNo < -53 || 0 == (int) $byWeekNo || (int) $byWeekNo > 53) {
                            throw new \WappoVendor\Sabre\VObject\InvalidDataException('BYWEEKNO in RRULE must have value(s) from 1 to 53, or -53 to -1!');
                        }
                    }
                    break;
                case 'BYMONTH':
                    ...
                    foreach ($this->byMonth as $byMonth) {
                        if (!\is_numeric($byMonth) || (int) $byMonth < 1 || (int) $byMonth > 12) {
                            throw new \WappoVendor\Sabre\VObject\InvalidDataException('BYMONTH in RRULE must have value(s) between 1 and 12!');
                        }
                    }
                    break;
                case 'BYSETPOS':
                ...
                case 'WKST':
                    ...
                    break;
                default:
                    throw new \WappoVendor\Sabre\VObject\InvalidDataException('Not supported: ' . \strtoupper($key));
            }
        }
    }
    /**
