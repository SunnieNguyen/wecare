...
class EventIterator implements \Iterator
{
    ...
    public function __construct($input, $uid = null, \DateTimeZone $timeZone = null)
    {
        ...
        if (\is_array($input)) {
            $events = $input;
        } elseif ($input instanceof \WappoVendor\Sabre\VObject\Component\VEvent) {
        ...
        if (isset($this->masterEvent->RDATE)) {
            $this->recurIterator = new \WappoVendor\Sabre\VObject\Recur\RDateIterator($this->masterEvent->RDATE->getParts(), $this->startDate);
        } elseif (isset($this->masterEvent->RRULE)) {
            $this->recurIterator = new \WappoVendor\Sabre\VObject\Recur\RRuleIterator($this->masterEvent->RRULE->getParts(), $this->startDate);
        } else {
            $this->recurIterator = new \WappoVendor\Sabre\VObject\Recur\RRuleIterator(['FREQ' => 'DAILY', 'COUNT' => 1], $this->startDate);
        }
        $this->rewind();
        if (!$this->valid()) {
            throw new \WappoVendor\Sabre\VObject\Recur\NoInstancesException('This recurrence rule does not generate any valid instances');
        }
    }
    /**
    ...
    {
        if ($this->counter > \WappoVendor\Sabre\VObject\Settings::$maxRecurrences && -1 !== \WappoVendor\Sabre\VObject\Settings::$maxRecurrences) {
            throw new \WappoVendor\Sabre\VObject\Recur\MaxInstancesExceededException('Recurring events are only allowed to generate ' . \WappoVendor\Sabre\VObject\Settings::$maxRecurrences);
        }
