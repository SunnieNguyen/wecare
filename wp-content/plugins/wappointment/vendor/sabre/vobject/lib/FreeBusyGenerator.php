...
class FreeBusyGenerator
{
     ...
     */
    public function setBaseObject(\WappoVendor\Sabre\VObject\Document $vcalendar)
    {
     ...
     * @param Document $vcalendar
     */
    public function setVAvailability(\WappoVendor\Sabre\VObject\Document $vcalendar)
    {
    ...
    public function setObjects($objects)
    {
        ...
        foreach ($objects as $object) {
            if (\is_string($object) || \is_resource($object)) {
                $this->objects[] = \WappoVendor\Sabre\VObject\Reader::read($object);
            } elseif ($object instanceof \WappoVendor\Sabre\VObject\Component) {
                $this->objects[] = $object;
            } else {
    ...
    {
        if (!$start) {
            $start = new \DateTimeImmutable(\WappoVendor\Sabre\VObject\Settings::$minDate);
        }
        if (!$end) {
            $end = new \DateTimeImmutable(\WappoVendor\Sabre\VObject\Settings::$maxDate);
        }
        $this->start = $start;
        $this->end = $end;
    }
    ...
    {
        $fbData = new \WappoVendor\Sabre\VObject\FreeBusyData($this->start->getTimeStamp(), $this->end->getTimeStamp());
     ...
     */
    protected function calculateAvailability(\WappoVendor\Sabre\VObject\FreeBusyData $fbData, \WappoVendor\Sabre\VObject\Component\VCalendar $vavailability)
    {
            ...
            if (isset($vavail->AVAILABLE)) {
                foreach ($vavail->AVAILABLE as $available) {
                    ...
                    if ($available->RRULE) {
                        ...
                        $rruleIterator = new \WappoVendor\Sabre\VObject\Recur\RRuleIterator($available->RRULE->getValue(), $availStart);
     ...
     */
    protected function calculateBusy(\WappoVendor\Sabre\VObject\FreeBusyData $fbData, array $objects)
    {
            ...
            foreach ($object->getBaseComponents() as $component) {
                switch ($component->name) {
                    case 'VEVENT':
                        ...
                        if ($component->RRULE) {
                            try {
                                $iterator = new \WappoVendor\Sabre\VObject\Recur\EventIterator($object, (string) $component->UID, $this->timeZone);
                            ...
                            $maxRecurrences = \WappoVendor\Sabre\VObject\Settings::$maxRecurrences;
                            ...
                            if (isset($component->DTEND)) {
                                ...
                                $duration = \WappoVendor\Sabre\VObject\DateTimeParser::parseDuration((string) $component->DURATION);
                    ...
                    case 'VFREEBUSY':
                        foreach ($component->FREEBUSY as $freebusy) {
                            ...
                            foreach ($values as $value) {
                                ...
                                $startTime = \WappoVendor\Sabre\VObject\DateTimeParser::parseDateTime($startTime);
                                if ('P' === \substr($endTime, 0, 1) || '-P' === \substr($endTime, 0, 2)) {
                                    $duration = \WappoVendor\Sabre\VObject\DateTimeParser::parseDuration($endTime);
                                    $endTime = clone $startTime;
                                    $endTime = $endTime->add($duration);
                                } else {
                                    $endTime = \WappoVendor\Sabre\VObject\DateTimeParser::parseDateTime($endTime);
                                }
     ...
     * @return VCalendar
     */
    protected function generateFreeBusyCalendar(\WappoVendor\Sabre\VObject\FreeBusyData $fbData)
    {
        if ($this->baseObject) {
            $calendar = $this->baseObject;
        } else {
            $calendar = new \WappoVendor\Sabre\VObject\Component\VCalendar();
        }
