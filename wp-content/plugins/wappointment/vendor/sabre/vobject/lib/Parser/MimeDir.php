 ...
 */
class MimeDir extends \WappoVendor\Sabre\VObject\Parser\Parser
{
    /**
     * The input stream.
     *
    ...
    protected function parseLine($line)
    {
        ...
        if ('BEGIN:' === \strtoupper(\substr($line, 0, 6))) {
            if (\substr($line, 6) === $this->root->name) {
                throw new \WappoVendor\Sabre\VObject\ParseException('Invalid MimeDir file. Unexpected component: "' . $line . '" in document type ' . $this->root->name);
            }
            ...
            if ($name !== $component->name) {
                throw new \WappoVendor\Sabre\VObject\ParseException('Invalid MimeDir file. expected: "END:' . $component->name . '" got: "END:' . $name . '"');
            }
            return $component;
        } else {
            // Property reader
        ...
        if (!\is_null($this->lineBuffer)) {
            ...
            do {
                ...
                if ($eof || \feof($this->input) && false === $rawLine) {
                    throw new \WappoVendor\Sabre\VObject\EofException('End of document reached prematurely');
                }
                if (false === $rawLine) {
                    throw new \WappoVendor\Sabre\VObject\ParseException('Error reading from input stream');
                }
    ...
    protected function readProperty($line)
    {
        ...
        if ('QUOTED-PRINTABLE' === \strtoupper($propObj['ENCODING'])) {
            ...
            if (\WappoVendor\Sabre\VObject\Document::VCARD21 === $this->root->getDocumentType() && isset($propObj['CHARSET'])) {
