 ...
 */
class XML extends \WappoVendor\Sabre\VObject\Parser\Parser
{
    ...
    public function parse($input = null, $options = 0)
    {
        ...
        if (\is_null($this->input)) {
            throw new \WappoVendor\Sabre\VObject\EofException('End of input stream, or no input supplied');
        }
        switch ($this->input['name']) {
            case '{' . self::XCAL_NAMESPACE . '}icalendar':
                $this->root = new \WappoVendor\Sabre\VObject\Component\VCalendar([], false);
            ...
            case '{' . self::XCARD_NAMESPACE . '}vcards':
                foreach ($this->input['value'] as &$vCard) {
                    $this->root = new \WappoVendor\Sabre\VObject\Component\VCard(['version' => '4.0'], false);
                    $this->pointer =& $vCard;
                ...
                break;
            default:
                throw new \WappoVendor\Sabre\VObject\ParseException('Unsupported XML standard');
        }
        return $this->root;
    }
    /**
     ...
     */
    protected function parseVCalendarComponents(\WappoVendor\Sabre\VObject\Component $parentComponent)
    {
     ...
     * @param Component $parentComponent
     */
    protected function parseVCardComponents(\WappoVendor\Sabre\VObject\Component $parentComponent)
    {
     ...
     */
    protected function parseProperties(\WappoVendor\Sabre\VObject\Component $parentComponent, $propertyNamePrefix = '')
    {
        foreach ($this->pointer ?: [] as $xmlProperty) {
            list($namespace, $tagName) = \WappoVendor\Sabre\Xml\Service::parseClarkNotation($xmlProperty['name']);
            ...
            $propertyNameExtended = ($this->root instanceof \WappoVendor\Sabre\VObject\Component\VCalendar ? 'xcal' : 'xcard') . ':' . $propertyName;
     ...
     * @param Component $parentComponent
     */
    protected function parseComponent(\WappoVendor\Sabre\VObject\Component $parentComponent)
    {
     ...
     * @param mixed     $value
     */
    protected function createProperty(\WappoVendor\Sabre\VObject\Component $parentComponent, $name, $parameters, $type, $value)
    {
    ...
    public function setInput($input)
    {
        ...
        if (\is_string($input)) {
            $reader = new \WappoVendor\Sabre\Xml\Reader();
            $reader->elementMap['{' . self::XCAL_NAMESPACE . '}period'] = 'WappoVendor\\Sabre\\VObject\\Parser\\XML\\Element\\KeyValue';
            $reader->elementMap['{' . self::XCAL_NAMESPACE . '}recur'] = 'WappoVendor\\Sabre\\VObject\\Parser\\XML\\Element\\KeyValue';
            $reader->xml($input);
            $input = $reader->parse();
        }
        $this->input = $input;
    ...
    {
        list(, $tagName) = \WappoVendor\Sabre\Xml\Service::parseClarkNotation($clarkedTagName);
        return $tagName;
    }
}
