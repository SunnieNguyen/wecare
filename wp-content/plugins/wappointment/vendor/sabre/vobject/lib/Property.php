 ...
 */
abstract class Property extends \WappoVendor\Sabre\VObject\Node
{
    /**
     * Property name.
     *
     ...
     */
    public function __construct(\WappoVendor\Sabre\VObject\Component $root, $name, $value = null, array $parameters = [], $group = null)
    {
        $this->name = $name;
        $this->group = $group;
        $this->root = $root;
    ...
    public function add($name, $value = null)
    {
        $noName = false;
        if (null === $name) {
            $name = \WappoVendor\Sabre\VObject\Parameter::guessParameterNameByValue($value);
            $noName = true;
        }
        if (isset($this->parameters[\strtoupper($name)])) {
            $this->parameters[\strtoupper($name)]->addValue($value);
        } else {
            $param = new \WappoVendor\Sabre\VObject\Parameter($this->root, $name, $value);
            $param->noName = $noName;
     ...
     * @param Xml\Writer $writer XML writer
     */
    protected function xmlSerializeValue(\WappoVendor\Sabre\Xml\Writer $writer)
    {
    ...
    public function validate($options = 0)
    {
        ...
        if (!\WappoVendor\Sabre\VObject\StringUtil::isUTF8($this->getRawMimeDirValue())) {
            $oldValue = $this->getRawMimeDirValue();
            $level = 3;
            if ($options & self::REPAIR) {
                $newValue = \WappoVendor\Sabre\VObject\StringUtil::convertToUTF8($oldValue);
                if (true || \WappoVendor\Sabre\VObject\StringUtil::isUTF8($newValue)) {
        ...
        if ($encoding = $this->offsetGet('ENCODING')) {
            if (\WappoVendor\Sabre\VObject\Document::VCARD40 === $this->root->getDocumentType()) {
                $warnings[] = ['level' => 3, 'message' => 'ENCODING parameter is not valid in vCard 4.', 'node' => $this];
            } else {
                $encoding = (string) $encoding;
                $allowedEncoding = [];
                switch ($this->root->getDocumentType()) {
                    case \WappoVendor\Sabre\VObject\Document::ICALENDAR20:
                        $allowedEncoding = ['8BIT', 'BASE64'];
                        break;
                    case \WappoVendor\Sabre\VObject\Document::VCARD21:
                        $allowedEncoding = ['QUOTED-PRINTABLE', 'BASE64', '8BIT'];
                        break;
                    case \WappoVendor\Sabre\VObject\Document::VCARD30:
                        $allowedEncoding = ['B'];
