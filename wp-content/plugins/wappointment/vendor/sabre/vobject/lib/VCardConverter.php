...
class VCardConverter
{
     ...
     */
    public function convert(\WappoVendor\Sabre\VObject\Component\VCard $input, $targetVersion)
    {
        $inputVersion = $input->getDocumentType();
        if ($inputVersion === $targetVersion) {
            return clone $input;
        }
        if (!\in_array($inputVersion, [\WappoVendor\Sabre\VObject\Document::VCARD21, \WappoVendor\Sabre\VObject\Document::VCARD30, \WappoVendor\Sabre\VObject\Document::VCARD40])) {
            throw new \InvalidArgumentException('Only vCard 2.1, 3.0 and 4.0 are supported for the input data');
        }
        if (!\in_array($targetVersion, [\WappoVendor\Sabre\VObject\Document::VCARD30, \WappoVendor\Sabre\VObject\Document::VCARD40])) {
            throw new \InvalidArgumentException('You can only use vCard 3.0 or 4.0 for the target version');
        }
        $newVersion = \WappoVendor\Sabre\VObject\Document::VCARD40 === $targetVersion ? '4.0' : '3.0';
        $output = new \WappoVendor\Sabre\VObject\Component\VCard(['VERSION' => $newVersion]);
     ...
     * @param int             $targetVersion
     */
    protected function convertProperty(\WappoVendor\Sabre\VObject\Component\VCard $input, \WappoVendor\Sabre\VObject\Component\VCard $output, \WappoVendor\Sabre\VObject\Property $property, $targetVersion)
    {
        ...
        if (\WappoVendor\Sabre\VObject\Document::VCARD30 !== $targetVersion && 'PHONE-NUMBER' === $valueType) {
            $valueType = null;
        }
        ...
        if (\WappoVendor\Sabre\VObject\Document::VCARD30 === $targetVersion) {
            if ($property instanceof \WappoVendor\Sabre\VObject\Property\Uri && \in_array($property->name, ['PHOTO', 'LOGO', 'SOUND'])) {
                $newProperty = $this->convertUriToBinary($output, $newProperty);
            } elseif ($property instanceof \WappoVendor\Sabre\VObject\Property\VCard\DateAndOrTime) {
                ...
                // uses.
                $parts = \WappoVendor\Sabre\VObject\DateTimeParser::parseVCardDateTime($property->getValue());
        ...
        } elseif (\WappoVendor\Sabre\VObject\Document::VCARD40 === $targetVersion) {
            // These properties were removed in vCard 4.0
            if (\in_array($property->name, ['NAME', 'MAILER', 'LABEL', 'CLASS'])) {
                return;
            }
            if ($property instanceof \WappoVendor\Sabre\VObject\Property\Binary) {
                $newProperty = $this->convertBinaryToUri($output, $newProperty, $parameters);
            } elseif ($property instanceof \WappoVendor\Sabre\VObject\Property\VCard\DateAndOrTime && isset($parameters['X-APPLE-OMIT-YEAR'])) {
        ...
        if (\WappoVendor\Sabre\VObject\Document::VCARD40 === $targetVersion) {
     ...
     * @return Property\Uri
     */
    protected function convertBinaryToUri(\WappoVendor\Sabre\VObject\Component\VCard $output, \WappoVendor\Sabre\VObject\Property\Binary $newProperty, array &$parameters)
    {
     ...
     */
    protected function convertUriToBinary(\WappoVendor\Sabre\VObject\Component\VCard $output, \WappoVendor\Sabre\VObject\Property\Uri $newProperty)
    {
        $value = $newProperty->getValue();
     ...
     */
    protected function convertParameters40(\WappoVendor\Sabre\VObject\Property $newProperty, array $parameters)
    {
     ...
     * @param array    $parameters
     */
    protected function convertParameters30(\WappoVendor\Sabre\VObject\Property $newProperty, array $parameters)
    {
        // Adding all parameters.
        foreach ($parameters as $param) {
            // vCard 2.1 allowed parameters with no name
