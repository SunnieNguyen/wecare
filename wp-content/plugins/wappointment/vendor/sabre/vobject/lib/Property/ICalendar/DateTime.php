 ...
 */
class DateTime extends \WappoVendor\Sabre\VObject\Property
{
    /**
    ...
    public function getDateTimes(\DateTimeZone $timeZone = null)
    {
        ...
        $tzid = $this['TZID'];
        if ($tzid) {
            $timeZone = \WappoVendor\Sabre\VObject\TimeZoneUtil::getTimeZone((string) $tzid, $this->root);
        }
        $dts = [];
        foreach ($this->getParts() as $part) {
            $dts[] = \WappoVendor\Sabre\VObject\DateTimeParser::parse($part, $timeZone);
        }
        return $dts;
    }
    /**
    ...
    public function validate($options = 0)
    {
        ...
        try {
            foreach ($values as $value) {
                switch ($valueType) {
                    case 'DATE':
                        \WappoVendor\Sabre\VObject\DateTimeParser::parseDate($value);
                        break;
                    case 'DATE-TIME':
                        \WappoVendor\Sabre\VObject\DateTimeParser::parseDateTime($value);
                        break;
                }
            }
