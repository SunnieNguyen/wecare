 ...
 */
class Recur extends \WappoVendor\Sabre\VObject\Property
{
    /**
    ...
    public function setValue($value)
    {
        ...
        if ($value instanceof \WappoVendor\StdClass) {
            $value = (array) $value;
        }
        if (\is_array($value)) {
            $newVal = [];
    ...
    public function getJsonValue()
    {
        ...
        foreach ($this->getParts() as $k => $v) {
            if (0 === \strcmp($k, 'UNTIL')) {
                $date = new \WappoVendor\Sabre\VObject\Property\ICalendar\DateTime($this->root, null, $v);
