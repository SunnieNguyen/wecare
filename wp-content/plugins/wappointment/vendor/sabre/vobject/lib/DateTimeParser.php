...
class DateTimeParser
{
    ...
    public static function parseDateTime($dt, \DateTimeZone $tz = null)
    {
        ...
        if (!$result) {
            throw new \WappoVendor\Sabre\VObject\InvalidDataException('The supplied iCalendar datetime value is incorrect: ' . $dt);
        }
    ...
    public static function parseDate($date, \DateTimeZone $tz = null)
    {
        ...
        if (!$result) {
            throw new \WappoVendor\Sabre\VObject\InvalidDataException('The supplied iCalendar date value is incorrect: ' . $date);
        }
        if (\is_null($tz)) {
            $tz = new \DateTimeZone('UTC');
        }
    ...
    public static function parseDuration($duration, $asString = false)
    {
        ...
        if (!$result) {
            throw new \WappoVendor\Sabre\VObject\InvalidDataException('The supplied iCalendar duration value is incorrect: ' . $duration);
        }
        if (!$asString) {
            $invert = false;
    ...
    public static function parseVCardDateTime($date)
    {
        ...
        if (!\preg_match($regex, $date, $matches)) {
            ...
            $regex = '/^
                ...
                $/x';
            if (!\preg_match($regex, $date, $matches)) {
                throw new \WappoVendor\Sabre\VObject\InvalidDataException('Invalid vCard date-time string: ' . $date);
            }
        }
    ...
    public static function parseVCardTime($date)
    {
        ...
        if (!\preg_match($regex, $date, $matches)) {
            ...
            $regex = '/^
                ...
                $/x';
            if (!\preg_match($regex, $date, $matches)) {
                throw new \WappoVendor\Sabre\VObject\InvalidDataException('Invalid vCard time string: ' . $date);
            }
        }
