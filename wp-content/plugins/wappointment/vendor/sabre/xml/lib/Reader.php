        ...
        try {
            while (self::ELEMENT !== $this->nodeType) {
                if (!$this->read()) {
                    ...
                    \libxml_clear_errors();
                    if ($errors) {
                        throw new \WappoVendor\Sabre\Xml\LibXMLException($errors);
                    }
                }
            }
    ...
    public function parseInnerTree(array $elementMap = null)
    {
        ...
        try {
            if (!$this->read()) {
                ...
                throw new \WappoVendor\Sabre\Xml\ParseException('This should never happen (famous last words)');
            }
            while (true) {
                if (!$this->isValid()) {
                    $errors = \libxml_get_errors();
                ...
                switch ($this->nodeType) {
                    ...
                    case self::END_ELEMENT:
                        ...
                        break 2;
                    case self::NONE:
                        throw new \WappoVendor\Sabre\Xml\ParseException('We hit the end of the document prematurely. This likely means that some parser "eats" too many elements. Do not attempt to continue parsing.');
                    default:
