...
class Service
{
     ...
     */
    public function getReader() : \WappoVendor\Sabre\Xml\Reader
    {
        $r = new \WappoVendor\Sabre\Xml\Reader();
     ...
     */
    public function getWriter() : \WappoVendor\Sabre\Xml\Writer
    {
        $w = new \WappoVendor\Sabre\Xml\Writer();
    ...
    public function parse($input, string $contextUri = null, string &$rootElementName = null)
    {
        if (\is_resource($input)) {
            ...
            if ('' === $input) {
                throw new \WappoVendor\Sabre\Xml\ParseException('The input element to parse is empty. Do not attempt to parse');
            }
        }
        $r = $this->getReader();
    ...
    public function expect($rootElementName, $input, string $contextUri = null)
    {
        ...
        if (!\in_array($result['name'], $rootElementName, true)) {
            throw new \WappoVendor\Sabre\Xml\ParseException('Expected ' . \implode(' or ', (array) $rootElementName) . ' but received ' . $result['name'] . ' as the root element');
        }
        return $result['value'];
    }
    /**
    ...
    public function mapValueObject(string $elementName, string $className)
    {
        ...
        $this->elementMap[$elementName] = function (\WappoVendor\Sabre\Xml\Reader $reader) use($className, $namespace) {
            return \WappoVendor\Sabre\Xml\Deserializer\valueObject($reader, $className, $namespace);
        };
        $this->classMap[$className] = function (\WappoVendor\Sabre\Xml\Writer $writer, $valueObject) use($namespace) {
