 ...
 */
function keyValue(\WappoVendor\Sabre\Xml\Reader $reader, string $namespace = null) : array
{
    ...
    if (\WappoVendor\Sabre\Xml\Reader::END_ELEMENT === $reader->nodeType) {
        $reader->next();
        return [];
    }
    $values = [];
    do {
        if (\WappoVendor\Sabre\Xml\Reader::ELEMENT === $reader->nodeType) {
    ...
    } while (\WappoVendor\Sabre\Xml\Reader::END_ELEMENT !== $reader->nodeType);
    $reader->read();
    return $values;
}
/**
 ...
 * @return string[]
 */
function enum(\WappoVendor\Sabre\Xml\Reader $reader, string $namespace = null) : array
{
    // If there's no children, we don't do anything.
    if ($reader->isEmptyElement) {
        $reader->next();
 ...
 * @return object
 */
function valueObject(\WappoVendor\Sabre\Xml\Reader $reader, string $className, string $namespace)
{
    ...
    $reader->read();
    do {
        if (\WappoVendor\Sabre\Xml\Reader::ELEMENT === $reader->nodeType && $reader->namespaceURI == $namespace) {
 ...
 */
function repeatingElements(\WappoVendor\Sabre\Xml\Reader $reader, string $childElementName) : array
{
 ...
 */
function mixedContent(\WappoVendor\Sabre\Xml\Reader $reader) : array
{
    // If there's no children, we don't do anything.
    if ($reader->isEmptyElement) {
        $reader->next();
    ...
    while (true) {
        ...
        } elseif ($reader->depth >= $previousDepth && \in_array($reader->nodeType, [\WappoVendor\Sabre\Xml\Reader::TEXT, \WappoVendor\Sabre\Xml\Reader::CDATA, \WappoVendor\Sabre\Xml\Reader::WHITESPACE])) {
            $content[] = $reader->value;
            $reader->read();
        } elseif (\WappoVendor\Sabre\Xml\Reader::END_ELEMENT == $reader->nodeType) {
