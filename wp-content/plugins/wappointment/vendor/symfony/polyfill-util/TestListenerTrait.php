...
class TestListenerTrait
{
    ...
    public function startTestSuite($mainSuite)
    {
        ...
        $SkippedTestError = \class_exists('WappoVendor\\PHPUnit\\Framework\\SkippedTestError') ? 'PHPUnit\\Framework\\SkippedTestError' : 'PHPUnit_Framework_SkippedTestError';
        foreach ($mainSuite->tests() as $suite) {
            ...
            if (!\preg_match('/^(.+)\\\\Tests(\\\\.*)Test$/', $testClass, $m)) {
                $mainSuite->addTest(\WappoVendor\Symfony\Polyfill\Util\TestListener::warning('Unknown naming convention for ' . $testClass));
                continue;
            }
            ...
            foreach (new \RegexIterator($bootstrap, '/define\\(\'/') as $defLine) {
                ...
                try {
                    eval($defLine);
                } catch (\WappoVendor\PHPUnit_Framework_Exception $ex) {
                    $warnings[] = \WappoVendor\Symfony\Polyfill\Util\TestListener::warning($ex->getMessage());
                } catch (\WappoVendor\PHPUnit\Framework\Exception $ex) {
                    $warnings[] = \WappoVendor\Symfony\Polyfill\Util\TestListener::warning($ex->getMessage());
                }
            }
            $bootstrap->rewind();
            foreach (new \RegexIterator($bootstrap, '/return p\\\\' . $testedClass->getShortName() . '::/') as $defLine) {
                if (!\preg_match('/^\\s*function (?P<name>[^\\(]++)(?P<signature>\\(.*\\)(?: ?: [^ ]++)?) \\{ (?<return>return p\\\\' . $testedClass->getShortName() . '::[^\\(]++)(?P<args>\\([^\\)]*+\\)); \\}$/', $defLine, $f)) {
                    $warnings[] = \WappoVendor\Symfony\Polyfill\Util\TestListener::warning('Invalid line in bootstrap.php: ' . \trim($defLine));
                    continue;
                }
...
EOPHP
            ...
            if (!$warnings && null === $defLine) {
                ...
                $mainSuite->addTest(new \WappoVendor\Symfony\Polyfill\Util\TestListener($suite));
            }
        }
