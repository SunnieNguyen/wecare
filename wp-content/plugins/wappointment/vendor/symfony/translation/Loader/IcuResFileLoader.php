 ...
 * @author stealth35
 */
class IcuResFileLoader implements \WappoVendor\Symfony\Component\Translation\Loader\LoaderInterface
{
    /**
     * {@inheritdoc}
     */
    public function load($resource, $locale, $domain = 'messages')
    {
        ...
        if (!$rb) {
            throw new \WappoVendor\Symfony\Component\Translation\Exception\InvalidResourceException(\sprintf('Cannot load resource "%s".', $resource));
        } elseif (\intl_is_failure($rb->getErrorCode())) {
            throw new \WappoVendor\Symfony\Component\Translation\Exception\InvalidResourceException($rb->getErrorMessage(), $rb->getErrorCode());
        }
        $messages = $this->flatten($rb);
        $catalogue = new \WappoVendor\Symfony\Component\Translation\MessageCatalogue($locale);
        $catalogue->add($messages, $domain);
        if (\class_exists('WappoVendor\\Symfony\\Component\\Config\\Resource\\DirectoryResource')) {
            $catalogue->addResource(new \WappoVendor\Symfony\Component\Config\Resource\DirectoryResource($resource));
        }
        return $catalogue;
    }
    /**
