...
class Request
{
    ...
    {
        $this->request = new \WappoVendor\Symfony\Component\HttpFoundation\ParameterBag($request);
        $this->query = new \WappoVendor\Symfony\Component\HttpFoundation\ParameterBag($query);
        $this->attributes = new \WappoVendor\Symfony\Component\HttpFoundation\ParameterBag($attributes);
        $this->cookies = new \WappoVendor\Symfony\Component\HttpFoundation\ParameterBag($cookies);
        $this->files = new \WappoVendor\Symfony\Component\HttpFoundation\FileBag($files);
        $this->server = new \WappoVendor\Symfony\Component\HttpFoundation\ServerBag($server);
        $this->headers = new \WappoVendor\Symfony\Component\HttpFoundation\HeaderBag($this->server->getHeaders());
    ...
    public static function createFromGlobals()
    {
        ...
        if (0 === \strpos($request->headers->get('CONTENT_TYPE'), 'application/x-www-form-urlencoded') && \in_array(\strtoupper($request->server->get('REQUEST_METHOD', 'GET')), ['PUT', 'DELETE', 'PATCH'])) {
            ...
            $request->request = new \WappoVendor\Symfony\Component\HttpFoundation\ParameterBag($data);
        }
        return $request;
    }
    /**
    ...
    public function duplicate(array $query = null, array $request = null, array $attributes = null, array $cookies = null, array $files = null, array $server = null)
    {
        $dup = clone $this;
        if (null !== $query) {
            $dup->query = new \WappoVendor\Symfony\Component\HttpFoundation\ParameterBag($query);
        }
        if (null !== $request) {
            $dup->request = new \WappoVendor\Symfony\Component\HttpFoundation\ParameterBag($request);
        }
        if (null !== $attributes) {
            $dup->attributes = new \WappoVendor\Symfony\Component\HttpFoundation\ParameterBag($attributes);
        }
        if (null !== $cookies) {
            $dup->cookies = new \WappoVendor\Symfony\Component\HttpFoundation\ParameterBag($cookies);
        }
        if (null !== $files) {
            $dup->files = new \WappoVendor\Symfony\Component\HttpFoundation\FileBag($files);
        }
        if (null !== $server) {
            $dup->server = new \WappoVendor\Symfony\Component\HttpFoundation\ServerBag($server);
            $dup->headers = new \WappoVendor\Symfony\Component\HttpFoundation\HeaderBag($dup->server->getHeaders());
        }
        $dup->languages = null;
        $dup->charsets = null;
        $dup->encodings = null;
    ...
    public static function setTrustedProxies(array $proxies)
    {
        ...
        if (2 > \func_num_args()) {
            @\trigger_error(\sprintf('The %s() method expects a bit field of Request::HEADER_* as second argument since Symfony 3.3. Defining it will be required in 4.0. ', __METHOD__), \E_USER_DEPRECATED);
            return;
        }
    ...
    {
        @\trigger_error(\sprintf('The "%s()" method is deprecated since Symfony 3.3 and will be removed in 4.0. Use the $trustedHeaderSet argument of the Request::setTrustedProxies() method instead.', __METHOD__), \E_USER_DEPRECATED);
        if ('forwarded' === $key) {
    ...
    public static function getTrustedHeaderName($key)
    {
        if (2 > \func_num_args() || \func_get_arg(1)) {
            @\trigger_error(\sprintf('The "%s()" method is deprecated since Symfony 3.3 and will be removed in 4.0. Use the Request::getTrustedHeaderSet() method instead.', __METHOD__), \E_USER_DEPRECATED);
        }
     ...
     */
    public function setSession(\WappoVendor\Symfony\Component\HttpFoundation\Session\SessionInterface $session)
    {
    ...
    public function getHost()
    {
        ...
        if ($host && '' !== \preg_replace('/(?:^\\[)?[a-zA-Z0-9-:\\]_]+\\.?/', '', $host)) {
            ...
            throw new \WappoVendor\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException(\sprintf('Invalid Host "%s".', $host));
        }
        if (\count(self::$trustedHostPatterns) > 0) {
            ...
            $this->isHostValid = false;
            throw new \WappoVendor\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException(\sprintf('Untrusted Host "%s".', $host));
        }
        return $host;
    }
    /**
    ...
    public function getMethod()
    {
        ...
        if (!\preg_match('/^[A-Z]++$/D', $method)) {
            throw new \WappoVendor\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException(\sprintf('Invalid method override "%s".', $method));
        }
        return $this->method = $method;
    }
    /**
    ...
    public function getLanguages()
    {
        ...
        $languages = \WappoVendor\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept-Language'))->all();
        $this->languages = [];
    ...
    public function getCharsets()
    {
        ...
        return $this->charsets = \array_keys(\WappoVendor\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept-Charset'))->all());
    }
    /**
    ...
    public function getEncodings()
    {
        ...
        return $this->encodings = \array_keys(\WappoVendor\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept-Encoding'))->all());
    }
    /**
    ...
    public function getAcceptableContentTypes()
    {
        ...
        return $this->acceptableContentTypes = \array_keys(\WappoVendor\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept'))->all());
    }
    /**
    ...
    {
        return self::$trustedProxies && \WappoVendor\Symfony\Component\HttpFoundation\IpUtils::checkIp($this->server->get('REMOTE_ADDR'), self::$trustedProxies);
    }
    private function getTrustedValues($type, $ip = null)
    {
        ...
        if (self::$trustedHeaders[$type] && $this->headers->has(self::$trustedHeaders[$type])) {
            foreach (\explode(',', $this->headers->get(self::$trustedHeaders[$type])) as $v) {
                $clientValues[] = (self::HEADER_CLIENT_PORT === $type ? '0.0.0.0:' : '') . \trim($v);
            }
        }
        ...
        throw new \WappoVendor\Symfony\Component\HttpFoundation\Exception\ConflictingHeadersException(\sprintf('The request has both a trusted "%s" header and a trusted "%s" header, conflicting with each other. You should either configure your proxy to remove one of them, or configure your project to distrust the offending one.', self::$trustedHeaders[self::HEADER_FORWARDED], self::$trustedHeaders[$type]));
    }
    private function normalizeAndFilterClientIps(array $clientIps, $ip)
    {
        ...
        foreach ($clientIps as $key => $clientIp) {
            ...
            if (\WappoVendor\Symfony\Component\HttpFoundation\IpUtils::checkIp($clientIp, self::$trustedProxies)) {
                unset($clientIps[$key]);
