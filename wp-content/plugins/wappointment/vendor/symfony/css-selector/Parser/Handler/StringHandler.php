 ...
 * @internal
 */
class StringHandler implements \WappoVendor\Symfony\Component\CssSelector\Parser\Handler\HandlerInterface
{
    private $patterns;
    private $escaping;
    ...
    public function handle(\WappoVendor\Symfony\Component\CssSelector\Parser\Reader $reader, \WappoVendor\Symfony\Component\CssSelector\Parser\TokenStream $stream)
    {
        ...
        if (!$match) {
            throw new \WappoVendor\Symfony\Component\CssSelector\Exception\InternalErrorException(\sprintf('Should have found at least an empty match at %d.', $reader->getPosition()));
        }
        // check unclosed strings
        if (\strlen($match[0]) === $reader->getRemainingLength()) {
            throw \WappoVendor\Symfony\Component\CssSelector\Exception\SyntaxErrorException::unclosedString($reader->getPosition() - 1);
        }
        ...
        $stream->push(new \WappoVendor\Symfony\Component\CssSelector\Parser\Token(\WappoVendor\Symfony\Component\CssSelector\Parser\Token::TYPE_STRING, $string, $reader->getPosition()));
