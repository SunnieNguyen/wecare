 ...
 */
namespace WappoVendor\Symfony\Polyfill\Php56;

use WappoVendor\Symfony\Polyfill\Util\Binary;
/**
 * @internal
 */
final class Php56
{
    ...
    public static function hash_equals($knownString, $userInput)
    {
        if (!\is_string($knownString)) {
            \trigger_error('Expected known_string to be a string, ' . \gettype($knownString) . ' given', \E_USER_WARNING);
            return false;
        }
        if (!\is_string($userInput)) {
            \trigger_error('Expected user_input to be a string, ' . \gettype($userInput) . ' given', \E_USER_WARNING);
            return false;
        }
        $knownLen = \WappoVendor\Symfony\Polyfill\Util\Binary::strlen($knownString);
        $userLen = \WappoVendor\Symfony\Polyfill\Util\Binary::strlen($userInput);
        if ($knownLen !== $userLen) {
            return false;
        }
        $result = 0;
    ...
    public static function ldap_escape($subject, $ignore = '', $flags = 0)
    {
        ...
        if (null === $charMaps) {
            $charMaps = array(self::LDAP_ESCAPE_FILTER => array('\\', '*', '(', ')', "\x00"), self::LDAP_ESCAPE_DN => array('\\', ',', '=', '+', '<', '>', ';', '"', '#', "\r"));
            $charMaps[0] = array();
            for ($i = 0; $i < 256; ++$i) {
