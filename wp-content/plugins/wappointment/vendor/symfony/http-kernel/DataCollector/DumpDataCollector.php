 ...
 */
class DumpDataCollector extends \WappoVendor\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \WappoVendor\Symfony\Component\VarDumper\Dumper\DataDumperInterface
{
    private $stopwatch;
    ...
    public function __construct(\WappoVendor\Symfony\Component\Stopwatch\Stopwatch $stopwatch = null, $fileLinkFormat = null, $charset = null, \WappoVendor\Symfony\Component\HttpFoundation\RequestStack $requestStack = null, \WappoVendor\Symfony\Component\VarDumper\Dumper\DataDumperInterface $dumper = null)
    {
    ...
    public function dump(\WappoVendor\Symfony\Component\VarDumper\Cloner\Data $data)
    {
        if ($this->stopwatch) {
            ...
            if (isset($trace[$i]['class'], $trace[$i]['function']) && 'dump' === $trace[$i]['function'] && 'Symfony\\Component\\VarDumper\\VarDumper' === $trace[$i]['class']) {
                ...
                while (++$i < 7) {
                    if (isset($trace[$i]['function'], $trace[$i]['file']) && empty($trace[$i]['class']) && 0 !== \strpos($trace[$i]['function'], 'call_user_func')) {
                        ...
                        $line = $trace[$i]['line'];
                        break;
                    } elseif (isset($trace[$i]['object']) && $trace[$i]['object'] instanceof \WappoVendor\Twig\Template) {
    ...
    public function collect(\WappoVendor\Symfony\Component\HttpFoundation\Request $request, \WappoVendor\Symfony\Component\HttpFoundation\Response $response, \Exception $exception = null)
    {
        ...
        if (!$this->requestStack || !$response->headers->has('X-Debug-Token') || $response->isRedirection() || $response->headers->has('Content-Type') && false === \strpos($response->headers->get('Content-Type'), 'html') || 'html' !== $request->getRequestFormat() || false === \strripos($response->getContent(), '</body>')) {
            if ($response->headers->has('Content-Type') && false !== \strpos($response->headers->get('Content-Type'), 'html')) {
                $this->dumper = new \WappoVendor\Symfony\Component\VarDumper\Dumper\HtmlDumper('php://output', $this->charset);
                $this->dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
            } else {
                $this->dumper = new \WappoVendor\Symfony\Component\VarDumper\Dumper\CliDumper('php://output', $this->charset);
            }
    ...
    public function getDumps($format, $maxDepthLimit = -1, $maxItemsPerDepth = -1)
    {
        ...
        if ('html' === $format) {
            $dumper = new \WappoVendor\Symfony\Component\VarDumper\Dumper\HtmlDumper($data, $this->charset);
    ...
    {
        if ($this->dumper instanceof \WappoVendor\Symfony\Component\VarDumper\Dumper\CliDumper) {
            $contextDumper = function ($name, $file, $line, $fmt) {
                if ($this instanceof \WappoVendor\Symfony\Component\VarDumper\Dumper\HtmlDumper) {
                    if ($file) {
                        $s = $this->style('meta', '%s');
            ...
            $cloner = new \WappoVendor\Symfony\Component\VarDumper\Cloner\VarCloner();
    ...
    {
        $html = '';
        $dumper = new \WappoVendor\Symfony\Component\VarDumper\Dumper\HtmlDumper(function ($line) use(&$html) {
            $html .= $line;
        }, $this->charset);
