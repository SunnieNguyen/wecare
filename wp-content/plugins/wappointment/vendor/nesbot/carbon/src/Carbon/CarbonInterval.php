...
class CarbonInterval extends \DateInterval
{
    ...
    {
        return static::$cascadeFactors ?: array('minutes' => array(\WappoVendor\Carbon\Carbon::SECONDS_PER_MINUTE, 'seconds'), 'hours' => array(\WappoVendor\Carbon\Carbon::MINUTES_PER_HOUR, 'minutes'), 'dayz' => array(\WappoVendor\Carbon\Carbon::HOURS_PER_DAY, 'hours'), 'months' => array(\WappoVendor\Carbon\Carbon::DAYS_PER_WEEK * \WappoVendor\Carbon\Carbon::WEEKS_PER_MONTH, 'dayz'), 'years' => array(\WappoVendor\Carbon\Carbon::MONTHS_PER_YEAR, 'months'));
    }
    ...
    {
        return static::getFactor('dayz', 'weeks') ?: \WappoVendor\Carbon\Carbon::DAYS_PER_WEEK;
    }
    /**
    ...
    {
        return static::getFactor('hours', 'dayz') ?: \WappoVendor\Carbon\Carbon::HOURS_PER_DAY;
    }
    /**
    ...
    {
        return static::getFactor('minutes', 'hours') ?: \WappoVendor\Carbon\Carbon::MINUTES_PER_HOUR;
    }
    /**
    ...
    {
        return static::getFactor('seconds', 'minutes') ?: \WappoVendor\Carbon\Carbon::SECONDS_PER_MINUTE;
    }
    /**
    ...
    public static function __callStatic($name, $args)
    {
        ...
        if (static::hasMacro($name)) {
            return \call_user_func_array(array(new static(0), $name), $args);
        }
    }
    /**
    ...
    protected static function translator()
    {
        if (static::$translator === null) {
            static::$translator = \WappoVendor\Carbon\Translator::get();
        }
     ...
     */
    public static function setTranslator(\WappoVendor\Symfony\Component\Translation\TranslatorInterface $translator)
    {
    ...
    {
        $periods = array('year' => array('y', $this->years), 'month' => array('m', $this->months), 'week' => array('w', $this->weeks), 'day' => array('d', $this->daysExcludeWeeks), 'hour' => array('h', $this->hours), 'minute' => array('min', $this->minutes), 'second' => array('s', $this->seconds));
        $parts = array();
    ...
    {
        return \WappoVendor\Carbon\CarbonPeriod::createFromArray(\array_merge(array($this), \func_get_args()));
    }
    /**
     * Invert the interval.
     *
    ...
    public function add(\DateInterval $interval)
    {
        ...
        if (($this->years || $this->months || $this->dayz || $this->hours || $this->minutes || $this->seconds) && $this->years <= 0 && $this->months <= 0 && $this->dayz <= 0 && $this->hours <= 0 && $this->minutes <= 0 && $this->seconds <= 0) {
            $this->years *= -1;
            $this->months *= -1;
            $this->dayz *= -1;
            $this->hours *= -1;
    ...
    {
        $date = \array_filter(array(static::PERIOD_YEARS => \abs($interval->y), static::PERIOD_MONTHS => \abs($interval->m), static::PERIOD_DAYS => \abs($interval->d)));
        $time = \array_filter(array(static::PERIOD_HOURS => \abs($interval->h), static::PERIOD_MINUTES => \abs($interval->i), static::PERIOD_SECONDS => \abs($interval->s)));
    ...
    {
        $current = \WappoVendor\Carbon\Carbon::now();
