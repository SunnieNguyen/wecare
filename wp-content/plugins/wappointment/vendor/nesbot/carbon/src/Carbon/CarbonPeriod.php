...
class CarbonPeriod implements \Iterator, \Countable
{
    ...
    {
        return \is_string($callable) && \substr($callable, 0, 2) === 'is' && (\method_exists('WappoVendor\\Carbon\\Carbon', $callable) || \WappoVendor\Carbon\Carbon::hasMacro($callable));
    }
    /**
    ...
    protected static function parseIso8601($iso)
    {
        ...
        foreach (\explode('/', $iso) as $key => $part) {
            ...
            } elseif ($interval === null && ($parsed = \WappoVendor\Carbon\CarbonInterval::make($part))) {
                $interval = $part;
            } elseif ($start === null && ($parsed = \WappoVendor\Carbon\Carbon::make($part))) {
                $start = $part;
            } elseif ($end === null && ($parsed = \WappoVendor\Carbon\Carbon::make(static::addMissingParts($start, $part)))) {
                $end = $part;
            } else {
    ...
    public function __construct()
    {
        ...
        foreach ($arguments as $argument) {
            if ($this->dateInterval === null && ($parsed = \WappoVendor\Carbon\CarbonInterval::make($argument))) {
                $this->setDateInterval($parsed);
            } elseif ($this->startDate === null && ($parsed = \WappoVendor\Carbon\Carbon::make($argument))) {
                $this->setStartDate($parsed);
            } elseif ($this->endDate === null && ($parsed = \WappoVendor\Carbon\Carbon::make($argument))) {
        ...
        if ($this->startDate === null) {
            $this->setStartDate(\WappoVendor\Carbon\Carbon::now());
        }
        if ($this->dateInterval === null) {
            $this->setDateInterval(\WappoVendor\Carbon\CarbonInterval::day());
    ...
    {
        if (!($interval = \WappoVendor\Carbon\CarbonInterval::make($interval))) {
    ...
    {
        if (!($date = \WappoVendor\Carbon\Carbon::make($date))) {
    ...
    {
        if (!\is_null($date) && !($date = \WappoVendor\Carbon\Carbon::make($date))) {
     ...
     * @return Carbon
     */
    protected function prepareForReturn(\WappoVendor\Carbon\Carbon $date)
    {
        $date = $date->copy();
        if ($this->timezone) {
    ...
    {
        $translator = \WappoVendor\Carbon\Carbon::getTranslator();
        $parts = array();
    ...
    public function __call($method, $parameters)
    {
        ...
        switch ($method) {
            ...
            case 'sinceNow':
                return $this->setStartDate(new \WappoVendor\Carbon\Carbon(), $first);
            case 'end':
            case 'until':
                return $this->setEndDate($first, $second);
            case 'untilNow':
                return $this->setEndDate(new \WappoVendor\Carbon\Carbon(), $first);
            case 'dates':
            case 'between':
            ...
            case 'second':
                return $this->setDateInterval(\call_user_func(
                    ...
                    array($this->isDefaultInterval ? new \WappoVendor\Carbon\CarbonInterval('PT0S') : $this->dateInterval, $method),
