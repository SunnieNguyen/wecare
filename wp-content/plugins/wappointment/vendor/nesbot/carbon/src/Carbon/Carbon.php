...
class Carbon extends \DateTime implements \JsonSerializable
{
     ...
     * @var string
     */
    const RFC7231_FORMAT = 'WappoVendor\\D, d M Y H:i:s \\G\\M\\T';
    /**
    ...
    protected static function safeCreateDateTimeZone($object)
    {
        ...
        $tz = @\timezone_open($object = (string) $object);
        if ($tz !== false) {
            return $tz;
        }
    ...
    public function __construct($time = null, $tz = null)
    {
        ...
        if ($isNow && !isset($testInstance) && static::isMicrosecondsFallbackEnabled() && (\version_compare(\PHP_VERSION, '7.1.0-dev', '<') || \version_compare(\PHP_VERSION, '7.1.3-dev', '>=') && \version_compare(\PHP_VERSION, '7.1.4-dev', '<'))) {
            ...
            // Use the timestamp returned by microtime as now can happen in the next second
    ...
    public static function createSafe($year = null, $month = null, $day = null, $hour = null, $minute = null, $second = null, $tz = null)
    {
        ...
        foreach ($fields as $field => $range) {
            if (${$field} !== null && (!\is_int(${$field}) || ${$field} < $range[0] || ${$field} > $range[1])) {
                throw new \WappoVendor\Carbon\Exceptions\InvalidDateException($field, ${$field});
            }
        }
    ...
    {
        return static::executeWithLocale($locale, function ($newLocale, \WappoVendor\Symfony\Component\Translation\TranslatorInterface $translator) {
    ...
    public static function getAvailableLocales()
    {
        ...
        $locales = array();
        if ($translator instanceof \WappoVendor\Carbon\Translator) {
    ...
    {
        return $this->copy()->setTimezone('UTC')->format('WappoVendor\\Y-m-d\\TH:i:s\\Z');
    }
    /**
    ...
    {
        $diff = \WappoVendor\Carbon\CarbonInterval::instance($diff, $trimMicroseconds);
        // @codeCoverageIgnoreStart
    ...
    {
        return $this->diffFiltered(\WappoVendor\Carbon\CarbonInterval::day(), $callback, $date, $absolute);
    }
    /**
    ...
    {
        return $this->diffFiltered(\WappoVendor\Carbon\CarbonInterval::hour(), $callback, $date, $absolute);
    }
    /**
     ...
     * @return int
     */
    public function diffFiltered(\WappoVendor\Carbon\CarbonInterval $ci, \Closure $callback, $date = null, $absolute = true)
    {
        $start = $this;
        ...
        $values = \array_filter(\iterator_to_array($period), function ($date) use($callback) {
            return \call_user_func($callback, \WappoVendor\Carbon\Carbon::instance($date));
        });
        $diff = \count($values);
    ...
    {
        return $this->diffInDaysFiltered(function (\WappoVendor\Carbon\Carbon $date) {
