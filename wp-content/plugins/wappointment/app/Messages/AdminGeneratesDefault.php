...
trait AdminGeneratesDefault
{
    public function getEmailContent($client, $appointment)
    {
        ...
        $date_start = $appointment->start_at->setTimezone($tz)->format(\Wappointment\Services\Settings::get('date_format'));
        $time_start = $appointment->start_at->setTimezone($tz)->format(\Wappointment\Services\Settings::get('time_format'));
        $time_end = $appointment->end_at->setTimezone($tz)->format(\Wappointment\Services\Settings::get('time_format'));
        $dataEmail = [
            '<u>' . $this->subject . '</u>',
    ...
    public function getStaffTz($appointment)
    {
        ...
        if (empty($tz)) {
            $tz = \Wappointment\Services\Settings::getStaff('timezone', 'UTC');
        }
        return $tz;
    }
}
