...
class AdminPendingAppointmentEmail extends \Wappointment\Messages\AbstractAdminEmail
{
    ...
    public function loadContent()
    {
        ...
        $this->addLogo();
        $this->addBr();
        $buttonConfirm = apply_filters('wappointment_appointment_pending_email_button', true, $this->params['appointment']->options);
        $lines = [
        ...
        if ($buttonConfirm === true) {
            ...
            $this->addButton(__('Confirm appointment', 'wappointment'), \Wappointment\WP\Helpers::adminUrl('wappointment_calendar&start=' . $st . '&end=' . $end . '&timezone=' . $tz . '&open_confirm=' . (int) $this->params['appointment']->id));
        } else {
