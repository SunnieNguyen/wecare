...
class EmailHelper
{
    ...
    public function getLinkEvent($view = 'new-event')
    {
        static $page_link = '';
        if (empty($page_link)) {
            $page_link = get_permalink(\Wappointment\Services\Settings::get('front_page'));
    ...
    {
        return \sprintf(__('Date: %s'), $order->created_at->format(\Wappointment\Services\Settings::get('date_format')));
    }
    protected static function getClientData($order)
    {
        $client_data = __('Client', 'wappointment') . '<br/>';
        $cfields = \Wappointment\Services\Settings::get('invoice_client');
    ...
    protected static function generateOrderTable($params)
    {
        if (!empty($params['order'])) {
            $order = \Wappointment\Models\Order::find($params['order']['id']);
        ...
        $rows = [];
        if (\Wappointment\Services\Settings::get('invoice')) {
            $rows[] = ['cells' => ['', '', \Wappointment\Services\Settings::get('invoice_num') . $order->id . '<br/>' . static::getOrderDate($order)], 'class' => '', 'cellClass' => 'muted'];
            $rows[] = ['cells' => [\nl2br(esc_html(\Wappointment\Services\Settings::get('invoice_seller'))), '', static::getClientData($order)], 'class' => 'linesep', 'cellClass' => 'muted'];
        }
        ...
        foreach ($order->prices as $price) {
            $rows[] = [$price->price->name, \Wappointment\Services\Payment::formatPrice($price->price->price / 100), $price->quantity];
        }
        if ($order->tax_amount > 0) {
            $rows[] = [__('Tax', 'wappointment'), \Wappointment\Services\Payment::formatPrice(\round($order->tax_amount / 100, 2)), ''];
        }
        $rows[] = ['cells' => [__('Total', 'wappointment'), \Wappointment\Services\Payment::formatPrice(($order->total + $order->tax_amount) / 100), ''], 'class' => 'bold lineb linet'];
        $rows[] = ['cells' => [__('Status', 'wappointment'), $order['payment_label'] . ' - ' . $order['status_label'], ''], 'class' => 'small'];
        return \Wappointment\Messages\Templates\Order::table($rows);
    }
