...
class AdminDailySummaryEmail extends \Wappointment\Messages\AbstractAdminEmail
{
    ...
    private function loadTomorrowData()
    {
        ...
        $this->date_string = $this->tomorrowCarbon()->format(\Wappointment\Services\Settings::get('date_format'));
        /* translators: %s - today's date. */
        $this->subject = \sprintf(__('Daily summary for %s', 'wappointment'), $this->date_string);
        $this->sections = new \Wappointment\Services\Sections($this->tomorrowCarbon()->timestamp, $this->tomorrowCarbon()->timestamp + 86400, $this->staff, $this->isLegacy());
    }
    public function loadContent()
    {
        ...
        $this->addLogo();
        $this->addBr();
        $serviceDurationInSeconds = \Wappointment\Services\Service::get()['duration'] * 60;
        ...
        if (!empty($coverage)) {
            $newlines = [
                ...
                \sprintf(__('Available slots: %1$s (duration %2$s min)', 'wappointment'), $this->sections->getFreeSlots($serviceDurationInSeconds), \Wappointment\Services\Service::get()['duration']),
        ...
        if ($this->sections->getFreeSlots($serviceDurationInSeconds) == 0) {
            $this->addButton(__('Open new slots', 'wappointment'), \Wappointment\WP\Helpers::adminUrl('wappointment_calendar'), false);
        }
    ...
    {
        return '<hr/>' . $appointment->start_at->setTimezone($this->tz)->format(\Wappointment\Services\Settings::get('time_format')) . ' ' . $appointment->getClientModel()->name . ' / ' . $appointment->getDuration() . ' - ' . $appointment->getLocation() . '<br>' . $appointment->getClientModel()->email;
    }
