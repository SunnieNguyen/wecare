...
abstract class AbstractAppointmentEmailJob extends \Wappointment\Jobs\AbstractEmailJob
{
    ...
    public function handle()
    {
        ...
        if (!empty($emailObject->body)) {
            $result = $this->transport->to($this->client->mailableAddress())->send($emailObject);
            if (!$result) {
    ...
    protected function generateContent()
    {
        ...
        $data = ['client' => $this->client, 'appointment' => $this->appointment, 'order' => $this->order];
        if ($this->reminder_id) {
