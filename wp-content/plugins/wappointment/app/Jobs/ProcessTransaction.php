...
class ProcessTransaction implements \Wappointment\Jobs\JobInterface
{
    private $appointment;
    private $api;
    private $event;
    public function __construct($params)
    {
        ...
        $this->api = new \Wappointment\Services\Wappointment\DotCom();
    ...
    private function setAppointment($appointmentArray)
    {
        if ($this->event == 'cancel') {
            ...
            $this->appointment = (new \Wappointment\Models\Appointment())->newFromBuilder($appointmentArray);
        } else {
            $this->appointment = \Wappointment\Models\Appointment::find($appointmentArray['id']);
        }
    }
}
