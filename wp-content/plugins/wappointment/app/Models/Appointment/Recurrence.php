...
class Recurrence
{
    ...
    public function __construct(\Wappointment\Models\Appointment $appointment)
    {
        $this->is_set = isset($appointment->options['recurrence']);
        $this->master = $appointment;
        $this->calendar = \Wappointment\Repositories\CalendarsBack::findById($this->master->staff_id);
    ...
    private function generateForDay(\Wappointment\ClassConnect\Carbon $start_temp)
    {
        //dayOfWeekIso
        $data_new = $this->master->toArray();
        $copy = \Wappointment\ClassConnect\Carbon::createFromTimestamp($this->master->start_at->timestamp)->setTimezone($this->calendar['timezone']);
        ...
        try {
            if (\Wappointment\Services\AppointmentNew::testExistingEvents($data_new['start_at'], $data_new['end_at'], $this->master->staff_id)) {
                \Wappointment\Models\Appointment::create($data_new);
            }
        } catch (\Throwable $th) {
            //throw $th;
        }
    ...
    private function needsGeneration(\Wappointment\ClassConnect\Carbon $start_temp)
    {
    ...
    public function dayList(\Wappointment\Models\Appointment $appointment)
    {
    ...
    protected function setRules(\Wappointment\Models\Appointment $appointment)
    {
        $this->check_days = $this->dayList($appointment);
    }
    private function setStart(\Wappointment\Models\Appointment $appointment)
    {
        $this->start = \Wappointment\ClassConnect\Carbon::createFromTimestamp($this->getStartTS($appointment))->setTimezone($this->calendar['timezone']);
    }
    ...
    {
        $this->end = \Wappointment\ClassConnect\Carbon::now($this->calendar['timezone'])->addDays($this->calendar['avb'])->endOfDay();
    }
