...
trait ManipulateLinks
{
    ...
    private function getPageLink($view = 'reschedule-event')
    {
        ...
        return (new \Wappointment\Messages\EmailHelper())->getLinkEvent($view) . (empty($editKey) ? '' : '&appointmentkey=' . $editKey) . $this->additionalLinkParams();
    }
    ...
    public function getMeetingLink()
    {
        ...
        if ($video_provider == 'jitsi') {
            return \Wappointment\Services\Jitsi::generate($this);
        }
    ...
    {
        $when_shows_link = (int) \Wappointment\Services\Settings::get('video_link_shows');
