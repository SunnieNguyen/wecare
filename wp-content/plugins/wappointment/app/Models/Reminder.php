...
class Reminder extends \Wappointment\ClassConnect\Model
{
    protected $table = 'wappo_reminders';
    protected $fillable = ['subject', 'type', 'event', 'locked', 'options', 'published'];
    protected $casts = ['options' => 'array', 'locked' => 'boolean', 'published' => 'boolean'];
    ...
    public function scopeActiveReminders($query)
    {
        ...
        if ((int) \Wappointment\Services\Settings::get('approval_mode') != 1 || \Wappointment\Services\Payment::active()) {
    ...
    {
        return new \Wappointment\Messages\AppointmentEmailFiller(['subject' => $this->subject, 'body' => $this->getHtmlBody($appointment)]);
    }
