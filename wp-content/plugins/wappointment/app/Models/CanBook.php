...
trait CanBook
{
    ...
    {
        $service = \Wappointment\Models\Service::find((int) $bookingRequest->get('service'));
        if (empty($service)) {
    ...
    public function book($bookingRequest, $forceConfirmed = false)
    {
        ...
        if (!$bookingRequest->get('service')) {
            return $this->bookLegacy($bookingRequest, $forceConfirmed);
            //legacy trick for older version of wappo-woo
        }
        ...
        if ($forceConfirmed) {
            $dataReturned = \Wappointment\Services\AppointmentNew::confirmedBook($this, $start_at, $end_at, false, $service, $staff);
        } else {
            $dataReturned = \Wappointment\Services\AppointmentNew::tryBook($this, $start_at, $end_at, false, $service, $staff);
        }
    ...
    public function bookAsAdmin($booking)
    {
        ...
        $end = $booking->get('end');
        //test that this is bookable
        $dataReturned = \Wappointment\Services\AppointmentNew::adminBook($this, $booking->get('start'), $end, 'unused', $booking->getService(), $booking->staff);
