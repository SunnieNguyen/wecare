...
class Client extends \Wappointment\ClassConnect\Model
{
    ...
    {
        $start_at_string = \Wappointment\ClassConnect\Carbon::now('UTC')->format(WAPPOINTMENT_DB_FORMAT);
        return \Wappointment\Models\Appointment::where('client_id', $this->id)->where('start_at', '>=', $start_at_string)->count();
    }
    ...
    {
        return ((int) $service['duration'] + (int) \Wappointment\Services\Settings::get('buffer_time')) * 60;
    }
    ...
    {
        return !empty($this->client->options['tax_percent']) ? $this->client->options['tax_percent'] : \Wappointment\Services\Settings::get('tax');
    }
