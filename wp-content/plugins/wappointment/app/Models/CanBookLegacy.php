...
trait CanBookLegacy
{
    public function bookLegacy($bookingRequest, $forceConfirmed = false)
    {
        ...
        if ($forceConfirmed) {
            $hasBeenBooked = \Wappointment\Services\Appointment::adminBook($this, $startTime, $startTime + $this->getRealDuration($service), $type, $service);
        } else {
            $hasBeenBooked = \Wappointment\Services\Appointment::tryBook($this, $startTime, $startTime + $this->getRealDuration($service), $type, $service);
        }
        if (!$hasBeenBooked) {
