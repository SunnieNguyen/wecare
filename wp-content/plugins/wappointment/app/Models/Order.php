...
class Order extends \Wappointment\ClassConnect\Model
{
    use SoftDeletes;
    ...
    {
        return \Wappointment\WP\Helpers::siteName() . ' - ' . !empty($this) ? $this->getDescription() : '';
    }
    ...
    {
        $this->currency = \Wappointment\Services\Payment::currencyCode();
    ...
    */
    public function cancelAllConnected()
    {
        foreach ($this->prices as $charge) {
            ...
            if ($charge->appointment_id > 0 && !\is_null($charge->appointment)) {
                ...
                $charge->save();
                \Wappointment\Services\Ticket::cancelTrigger(apply_filters('wappointment_appointment_get_ticket', $appointment, $this->client_id), $charge->quantity);
            }
        }
    }
    ...
    protected function cancelPreviousFromMeta($appointment)
    {
        ...
        \Wappointment\Services\AppointmentNew::silentCancel($appointment_ids, $charge_ids);
        if (!empty($reservations) && $appointment->id !== (int) $reservations[0]['appointment_id']) {
            \Wappointment\Jobs\CleanPendingPaymentAppointment::cancelReservations(\array_merge($this->options, ['client_id' => $this->client_id]));
        }
    }
    ...
    {
        foreach ($this->prices as $charge) {
            \Wappointment\Services\AppointmentNew::confirm($charge->appointment_id, true, $this->client, $this);
        }
    }
    ...
    public function arrayResult()
    {
        ...
        $arrayResult['appointments'] = \Wappointment\Formatters\BookingResult::formatAppointments($this->appointments);
        return $arrayResult;
    }
