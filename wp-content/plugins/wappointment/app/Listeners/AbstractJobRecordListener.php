...
abstract class AbstractJobRecordListener
{
    ...
    {
        $this->jobs[] = ['payload' => \json_encode(['job' => $jobClass, 'params' => $params]), 'queue' => $queue, 'appointment_id' => $appointment_id, 'created_at' => \time(), 'available_at' => $available_at];
    }
    protected function queueJobs()
    {
        if (!empty($this->jobs)) {
            \Wappointment\Services\Queue::pushMultiple($this->jobs);
        }
    }
