...
class CustomPage
{
    ...
    public function install()
    {
        ...
        \Wappointment\Services\Settings::save('front_page', $page_id, true);
    }
    ...
    public function makeEditablePage()
    {
        ...
        \Wappointment\Services\Settings::save('front_page', $pageid);
    }
    public function boot()
    {
        $showItf = false;
        register_post_type($this->slug, ['labels' => 'Wappointment', 'public' => true, 'has_archive' => false, 'show_ui' => $showItf, 'show_in_menu' => $showItf, 'rewrite' => false, 'show_in_nav_menus' => false, 'can_export' => false, 'publicly_queryable' => true, 'exclude_from_search' => true]);
    ...
    {
        if (!empty(\Wappointment\WP\Helpers::requestGet()->input('appointmentkey'))) {
            return true;
        }
        return !empty(\Wappointment\WP\Helpers::requestGet()->input($this->slug));
    }
    public function isAddEventToCalendarPage()
    {
        return \strpos(\Wappointment\WP\Helpers::requestGet()->input('view'), 'add-event-to-calendar') !== false;
    }
    public function isReschedulePage()
    {
        return \strpos(\Wappointment\WP\Helpers::requestGet()->input('view'), 'reschedule-event') !== false;
    }
    public function isCancelPage()
    {
        return \strpos(\Wappointment\WP\Helpers::requestGet()->input('view'), 'cancel-event') !== false;
    }
    public function isNewAppointmentPage()
    {
        return \Wappointment\WP\Helpers::requestGet()->input('view') == 'new-event';
    }
    public function isViewAppointmentPage()
    {
        return \Wappointment\WP\Helpers::requestGet()->input('view') == 'view-event';
    }
    public function getPageTitle()
    {
        ...
        if ($this->isNewAppointmentPage()) {
            return \Wappointment\Services\Settings::get('new_booking_link');
        }
    }
