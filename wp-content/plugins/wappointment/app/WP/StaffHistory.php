...
class StaffHistory extends \Wappointment\WP\AppointmentHistory
{
    public function loadData()
    {
        ...
        if (!empty($id)) {
            ...
            $this->appointments = \Wappointment\Models\Appointment::where('staff_id', $calendar['id'])->orderBy('id', 'DESC')->get();
        }
    }
    ...
    public function getDataClient($client)
    {
        ...
        $cfs = \Wappointment\Managers\Central::get('CustomFields')::get();
        foreach ($cfs as $cf) {
            if (empty($cf['core'])) {
