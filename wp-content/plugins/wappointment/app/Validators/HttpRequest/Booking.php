...
class Booking extends \Wappointment\Validators\HttpRequest\LegacyBooking
{
    ...
    {
        return $this->forceEmail() && $this->isLogged() && $this->bookingFromFront() ? \Wappointment\WP\Helpers::currentUserEmail() : $this->get('email');
    }
    ...
    {
        return \Wappointment\Services\Settings::get('forceemail');
    }
    public function isLogged()
    {
        return \Wappointment\WP\Helpers::auth();
    }
    ...
    {
        $location = \Wappointment\Models\Location::find((int) $this->request->input('location'));
        if (\Wappointment\Models\Location::TYPE_PHONE === $location->type) {
        ...
        $this->validator->addValidator('is_phone', new \Wappointment\Validators\Phone($countries));
    }
    public function validateService($inputs)
    {
        ...
        $service = \Wappointment\Managers\Service::model()::find((int) $inputs['service']);
        if (empty($service)) {
    ...
    public function prepareInputs($inputs) : array
    {
        if (\Wappointment\Services\VersionDB::canServices()) {
            $this->staff = \Wappointment\Models\Calendar::active()->findOrFail((int) $inputs['staff_id']);
        }
