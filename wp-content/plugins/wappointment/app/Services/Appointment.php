...
class Appointment
{
    ...
    public static function confirm($id)
    {
        ...
        if (empty($appointment)) {
            ...
            if ($result) {
                \Wappointment\Helpers\Events::dispatch('AppointmentConfirmedEvent', ['appointment' => $appointment, 'client' => \Wappointment\Models\Client::find($appointment->client_id), 'oldAppointment' => $oldAppointment]);
            }
            return $result;
        }
    }
    ...
    protected static function appointmentModified($appointment, $oldAppointment)
    {
        ...
        \Wappointment\Helpers\Events::dispatch('AppointmentRescheduledEvent', ['appointment' => $appointment, 'client' => $appointment->client()->first(), 'oldAppointment' => $oldAppointment]);
    }
    ...
    public static function tryBook(\Wappointment\Models\Client $client, $start_at, $end_at, $type, $service)
    {
        if (static::canBook($start_at, $end_at)) {
            return static::processBook($client, $start_at, $end_at, $type, $service);
        }
    }
    public static function adminBook(\Wappointment\Models\Client $client, $start_at, $end_at, $type, $service)
    {
        if (static::canBook($start_at, $end_at, true)) {
            return static::processBook($client, $start_at, $end_at, $type, $service, true);
        }
    }
    protected static function processBook(\Wappointment\Models\Client $client, $start_at, $end_at, $type, $service, $forceConfirmed = false)
    {
    ...
    protected static function book($data, \Wappointment\Models\Client $client, $is_admin = false)
    {
        $appointment = static::create($data);
        if ($appointment->status == static::getAppointmentModel()::STATUS_AWAITING_CONFIRMATION) {
            \Wappointment\Helpers\Events::dispatch('AppointmentBookedEvent', ['appointment' => $appointment, 'client' => $client]);
        } else {
            \Wappointment\Helpers\Events::dispatch('AppointmentConfirmedEvent', ['appointment' => $appointment, 'client' => $client]);
        }
        //return $appointment;
    ...
    public static function cancel(\Wappointment\Models\Appointment $appointment)
    {
        ...
        if ($result) {
            ...
            \Wappointment\Helpers\Events::dispatch('AppointmentCanceledEvent', ['appointment' => $appointment, 'client' => $client]);
        }
        return $result;
    }
