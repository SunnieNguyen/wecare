...
class ClientLegacy
{
    public static function book(\Wappointment\Validators\HttpRequest\LegacyBooking $booking)
    {
        if (\is_array($booking->get('email'))) {
            throw new \WappointmentException("Malformed parameter", 1);
        }
        //create or load client account
        $client = \Wappointment\Models\Client::withTrashed()->where('email', $booking->get('email'))->first();
