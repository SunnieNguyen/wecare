...
class CalendarParser
{
    ...
    public function handle()
    {
        ...
        foreach ($vcalendar->VEVENT as $vevent) {
            ...
            $until = null;
            $recur = \Wappointment\Models\Status::RECUR_NOT;
            ...
            if ($vevent->RRULE) {
                ...
                if (!empty($until) && $until->lt(\Wappointment\ClassConnect\Carbon::yesterday())) {
                    continue;
                }
                ...
                if ($carbon_end->lt(\Wappointment\ClassConnect\Carbon::yesterday())) {
                    continue;
                }
            }
        ...
        return ['detected' => \count($this->statusEvents), 'deleted' => $this->deleteRemovedEvents($uids), 'inserted' => $this->insertIgnoreOrUpsert($this->statusEvents->toArray()), 'duration' => \round(\microtime(true) - $start, 2)];
    }
    ...
    {
        return $this->handles_free && $this->freeSpotted($vevent) ? \Wappointment\Models\Status::TYPE_FREE : \Wappointment\Models\Status::TYPE_BUSY;
    }
    private function insertIgnoreOrUpsert($array)
    {
        return \version_compare(\Wappointment\System\Status::dbVersion(), '2.0.3') >= 0 ? \Wappointment\Models\Status::upsert($array) : \Wappointment\Models\Status::insertIgnore($array);
    }
    private function deleteRemovedEvents($uids)
    {
        $deleted = 0;
        if (\count($uids->unique()) === \count($uids)) {
            $deleted = \Wappointment\Models\Status::where('source', $this->source)->whereNotIn('eventkey', $this->statusEvents->pluck('eventkey'))->delete();
        }
        return $deleted;
    }
    private function getUntil($vevent)
    {
        return empty($vevent->RRULE->getParts()['UNTIL']) ? null : $this->vcalDateToCarbon($vevent->RRULE->getParts()['UNTIL'], $vevent, true);
    }
    private function getRecur($vevent)
    {
        return empty($vevent->RRULE->getParts()['FREQ']) ? \Wappointment\Models\Status::RECUR_NOT : $this->getFrequency($vevent->RRULE->getParts()['FREQ']);
    }
    ...
    private function vcalDateToCarbon($vcalDateTimeString, $vevent = null)
    {
        ...
        return \Wappointment\ClassConnect\Carbon::parse($vcalDateTimeString, $timezoneTemp);
    }
    ...
    protected function findTimezone($timezone_string)
    {
        if (!empty($timezone_string)) {
            $tzobject = \Wappointment\ClassConnect\VtzUtil::getTimeZone((string) $timezone_string);
    ...
    private function getFrequency($frequency)
    {
        switch (\strtoupper($frequency)) {
            case 'DAILY':
                return \Wappointment\Models\Status::RECUR_DAILY;
            case 'WEEKLY':
                return \Wappointment\Models\Status::RECUR_WEEKLY;
            case 'MONTHLY':
                return \Wappointment\Models\Status::RECUR_MONTHLY;
            case 'YEARLY':
                return \Wappointment\Models\Status::RECUR_YEARLY;
            default:
                return \Wappointment\Models\Status::RECUR_NOT;
        }
    }
    private function getOptions($vevent, $until, $recur)
    {
        ...
        if ($recur > \Wappointment\Models\Status::RECUR_NOT) {
            if (!empty($until)) {
