...
class ExternalCalendar
{
    ...
    public function __construct($staff_id = false)
    {
        ...
        if (!$this->isLegacy) {
            $this->staff = \Wappointment\Models\Calendar::findOrFail($staff_id);
        }
    }
    ...
    {
        if ($this->isLegacy) {
            $calendar_logs = \Wappointment\WP\Helpers::getStaffOption('calendar_logs');
            if (isset($calendar_logs[$calendar_key])) {
                unset($calendar_logs[$calendar_key]);
                \Wappointment\WP\Helpers::setStaffOption('calendar_logs', $calendar_logs);
            }
        } else {
    ...
    public function refreshCalendars($force = false)
    {
        ...
        return ['message' => \Wappointment\Helpers\Translations::get('element_refreshed')];
    }
    public function save($calurl)
    {
        ...
        $this->refreshCalendars(true);
        return ['message' => \Wappointment\Helpers\Translations::get('element_saved'), 'status' => $statusSaved];
    }
    ...
    {
        $calurls = \Wappointment\WP\Helpers::getStaffOption('cal_urls');
        if (empty($calurls)) {
            $calurls = [];
        }
        return $calurls;
    ...
    {
        return \Wappointment\WP\Helpers::setStaffOption('cal_urls', $calurls);
    }
    protected function calurlValid($value)
    {
        ...
        try {
            if ($this->isLegacy) {
                $result = (new \Wappointment\Services\Calendar($value, \Wappointment\Services\Settings::get('activeStaffId'), true))->refetch();
            } else {
