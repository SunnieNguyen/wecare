...
{
    public static function tryBook(\Wappointment\Models\Client $client, $start_at, $end_at, $type, $service, $staff_id = null)
    {
    ...
    public static function adminBook(\Wappointment\Models\Client $client, $start_at, $end_at, $type, $service, $staff_id = null)
    {
    ...
    public static function confirmedBook(\Wappointment\Models\Client $client, $start_at, $end_at, $type, $service, $staff_id = null)
    {
        if (static::canBook($start_at, $end_at, true, $staff_id)) {
            return static::processBook($client, $start_at, $end_at, $type, $service, true, $staff_id);
        }
    }
    protected static function processBook(\Wappointment\Models\Client $client, $start_at, $end_at, $type, $service, $forceConfirmed = false, $staff_id = null, $adminBooked = false)
    {
        $start_at = static::unixToDb($start_at);
        $end_at = static::unixToDb($end_at);
        $location = \Wappointment\Models\Location::where('id', $client->bookingRequest->get('location'))->first();
        $status = $forceConfirmed ? \Wappointment\Models\Appointment::STATUS_CONFIRMED : static::getDefaultStatus($service);
    ...
    public static function adminCalendarGetAppointments($params, $legacy = false)
    {
        ...
        $appointmentsQuery = \Wappointment\Models\Appointment::with($with)->where('status', '>=', \Wappointment\Models\Appointment::STATUS_AWAITING_CONFIRMATION)->where('start_at', '>=', $params['start_at'])->where('end_at', '<=', $params['end_at']);
    ...
    protected static function getAppointmentLocation($appointment)
    {
        if (!empty($appointment->location_id) && $appointment->location_id > 0) {
            return \Wappointment\Models\Location::where('id', $appointment->location_id)->first();
        }
    }
     ...
     */
    public static function create($data, \Wappointment\Models\Client $client, $status, $adminBooked = false)
    {
    ...
    public static function generateOrder($client, \Wappointment\Models\TicketAbstract $ticket, $slots = false, $service = false, $adminBooked = false)
    {
        $order = null;
    ...
    public static function confirm($id, $soft = false, $client = null, $order = null)
    {
        ...
        if (empty($appointment)) {
            ...
            if ($result) {
                ...
                $clientModel = empty($client) ? \Wappointment\Models\Client::find($appointment->client_id) : $client;
    ...
    protected static function appointmentModified($appointment, $oldAppointment)
    {
        ...
        \Wappointment\Services\JobHelper::dispatch('AppointmentRescheduledEvent', ['appointment' => $appointment, 'client' => $clientModel, 'oldAppointment' => $oldAppointment], $clientModel);
    }
    ...
    protected static function bookCreate($data, \Wappointment\Models\Client $client, $is_admin = false, $status = 0, $adminBooked = false)
    {
    ...
    public static function availabilityRefreshTrigger($staff_id, $is_admin = false)
    {
        ...
        if (!\defined('DISABLE_WP_CRON') || $is_admin === true) {
            ...
            \Wappointment\WP\Helpers::cronTrigger();
        }
    }
    ...
    public static function cancel(\Wappointment\Models\Appointment $appointment, $client = null, $force = false)
    {
    ...
    {
        $appointments = \Wappointment\Models\Appointment::whereIn('id', $appointment_ids)->get();
    ...
    {
        if (empty($charge_ids)) {
            $charge_ids = \Wappointment\Models\OrderPrice::select('id')->whereIn('appointment_id', $appointment_ids)->get()->map(function ($e) {
                return $e->id;
            })->toArray();
        }
        if (!empty($charge_ids)) {
            $query = \Wappointment\Models\OrderPrice::whereIn('id', $charge_ids);
            if ($delete) {
                $query->delete();
