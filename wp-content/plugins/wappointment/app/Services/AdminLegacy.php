...
class AdminLegacy
{
    public static function book(\Wappointment\Validators\HttpRequest\BookingAdmin $booking)
    {
        ...
        if ($client_id > 0) {
            $client = \Wappointment\Models\Client::find($client_id);
        } else {
            if (\is_array($booking->get('email'))) {
                throw new \WappointmentException(__('Malformed parameter', 'wappointment'), 1);
            }
            $client = \Wappointment\Models\Client::where('email', $booking->get('email'))->first();
        }
    ...
    private static function createAppointment(\Wappointment\Models\Client $client, $start, $end, $type, $service = false)
    {
        if ($service === false) {
