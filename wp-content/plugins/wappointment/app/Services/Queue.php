...
class Queue
{
    ...
    {
        return \Wappointment\Models\Job::where('reserved_at', 0)->where('available_at', '<', $reserve_at)->take($take)->update(['reserved_at' => $reserve_at]);
    }
    private static function loadBatch($take)
    {
        ...
        //and load it
        return \Wappointment\Models\Job::where('reserved_at', $reserve_at)->take($take)->get();
    }
    public static function pushMultiple($jobsObject)
    {
        return \Wappointment\Models\Job::insert($jobsObject);
    }
    /**
    ...
    {
        $query = \Wappointment\Models\Job::where('queue', $queue)->where('reserved_at', 0);
    ...
    public static function push($job, $params, $queue = 'default', $available_at = 0)
    {
        ...
        return \Wappointment\Models\Job::create($jobInsert);
    }
    public static function resetTimedoutJobs()
    {
        $jobsTimedOut = \Wappointment\Models\Job::where('reserved_at', '>', 0)->where('reserved_at', '<', \time() - static::DELAY_TIMEOUT_RETRY)->get();
    ...
    private static function run(\Wappointment\Models\Job $job)
    {
        //process it
    ...
    private static function whenJobFails(\Wappointment\Models\Job $job, \Exception $e)
    {
        $attempts = $job->attempts + 1;
        if ($attempts >= self::MAX_FAILURE) {
            // MAX_FAILURE reached we keep a record in the failures table and delete the original job
            self::jobFailed($job, \Wappointment\Helpers\Debug::convertExceptionToArray($e));
        } else {
            $job->update(['attempts' => $attempts, 'payload' => $job->payload, 'reserved_at' => 0, 'available_at' => \time() + self::DELAY_AFTER_FAIL]);
        }
    }
    protected static function jobFailed(\Wappointment\Models\Job $job, $arrayException)
    {
        ...
        \Wappointment\Models\FailedJob::create($failed_job);
        $job->delete();
    }
    public static function loadAndRun($jobId)
    {
        $job = \Wappointment\Models\Job::find($jobId);
        if (!$job) {
    ...
    {
        \Wappointment\Models\Job::where('queue', '=', 'client')->where('appointment_id', (int) $appointment_id)->delete();
    }
    public static function cancelJob($queue = 'daily', $staff_id_only = false)
    {
        $query = \Wappointment\Models\Job::where('queue', $queue);
        if ($staff_id_only) {
    ...
    {
        $site_timezone = \Wappointment\WP\Helpers::getWPOption('timezone_string');
