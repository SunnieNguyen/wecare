...
class Status
{
    ...
    {
        $statusObject = \Wappointment\Models\Status::find($id);
        if ($statusObject && !empty($statusObject->source)) {
            ...
            $status = \Wappointment\Models\Status::destroy($id);
        }
        if ($status) {
    ...
    public static function free($start, $end, $timezone, $request, $staff_id = null)
    {
        if (\Wappointment\Services\CurrentUser::canCreateFreeBlock()) {
            return self::create($start, $end, $timezone, \Wappointment\Models\Status::TYPE_FREE, $request, static::getAllowedStaffId($staff_id));
        }
    ...
    public static function busy($start, $end, $timezone, $staff_id = null)
    {
        if (\Wappointment\Services\CurrentUser::canCreateBusyBlock()) {
            return self::create($start, $end, $timezone, \Wappointment\Models\Status::TYPE_BUSY, null, static::getAllowedStaffId($staff_id));
        }
        throw new \WappointmentException(\Wappointment\Helpers\Translations::get('error_saving'), 1);
    }
    protected static function create($start, $end, $timezone, $type, $request = null, $staff_id = null)
    {
        ...
        $resultObject = \Wappointment\Models\Status::create($arrayCreate);
        if ($resultObject) {
    ...
    public static function tryConvertToUTCFormatted($timevalue, $timezone)
    {
        if (\is_numeric($timevalue)) {
            //unixtimestamp
            return \Wappointment\ClassConnect\Carbon::createFromTimestamp($timevalue)->format(WAPPOINTMENT_DB_FORMAT . ':00');
        } else {
    ...
    public static function canAdd($next)
    {
        if (isset($next->options['exdate'])) {
            foreach ($next->options['exdate'] as $exdate) {
                if (\Wappointment\ClassConnect\Carbon::createFromTimestamp($exdate)->timestamp === $next->start_at->timestamp) {
                    return false;
                }
            }
        }
    ...
    private static function getPreviousFrom($from, $recur)
    {
        $day_in_sec = 3600 * 24;
        switch ($recur) {
            case \Wappointment\Models\Status::RECUR_DAILY:
                return $from - $day_in_sec;
            case \Wappointment\Models\Status::RECUR_WEEKLY:
                return $from - $day_in_sec * 7;
            case \Wappointment\Models\Status::RECUR_MONTHLY:
            case \Wappointment\Models\Status::RECUR_YEARLY:
            default:
                return $from;
        }
    }
    ...
    private static function getNextDaily($statusRecurrent, $start_at, $from, $until)
    {
        ...
        $daysAdded = $start_at->timestamp > $from ? 0 : \Wappointment\ClassConnect\Carbon::createFromTimestamp($from)->diffInDays($start_at);
    ...
    private static function getNextWeekly($statusRecurrent, $start_at, $from, $until)
    {
        ...
        $daysAdded = $start_at->timestamp > \time() ? 0 : \Wappointment\ClassConnect\Carbon::now()->diffInDays($start_at);
        ...
        $origintimedate = \Wappointment\ClassConnect\Carbon::parse($statusRecurrent->options['origin_start'], $statusRecurrent->options['origin_tz']);
    ...
    private static function getNextMonthly($statusRecurrent, $start_at, $from, $until)
    {
        ...
        $diffInSeconds = $start_at->timestamp > \time() ? 0 : \Wappointment\ClassConnect\Carbon::now()->diffInSeconds($start_at);
        if ($diffInSeconds > 0) {
    ...
    private static function setDayOfTheMonth($carbon, $dayofthemonth, $statusRecurrent)
    {
        ...
        $carbonNewStart = \Wappointment\ClassConnect\Carbon::createFromTimestamp($carbon->timestamp, $statusRecurrent->options['origin_tz']);
    ...
    {
        $interval = self::getInterval($statusRecurrent);
        $yearsAdded = $start_at->timestamp > \time() ? 0 : \Wappointment\ClassConnect\Carbon::now()->diffInYears($start_at);
    ...
    private static function punctualStatusEvent($statusRecurrent, $start_at)
    {
        ...
        $newCopy->end_at = \Wappointment\ClassConnect\Carbon::createFromTimestamp($start_at->timestamp + self::$diff);
