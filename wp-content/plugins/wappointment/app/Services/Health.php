...
class Health
{
    ...
    {
        if (\Wappointment\Services\CurrentUser::isAdmin() && \Wappointment\System\Status::isInstalled() && \Wappointment\Models\Appointment::count() > 0) {
            $this->set('lang', \Wappointment\Helpers\Get::list('translations_health'));
    ...
    protected function pendingTasks()
    {
        ...
        $late = \time() - 60;
        $jobs = \Wappointment\Models\Job::where('available_at', '<', $late)->get();
