...
class DotCom extends \Wappointment\Services\Wappointment\API
{
    ...
    public function setStaff($staff_id = false)
    {
        if ($staff_id !== false) {
            ...
            if (!$this->isLegacy) {
                $this->staff = \Wappointment\Managers\Central::get('CalendarModel')::findOrFail($this->staff_id);
            }
            $this->setAccountKey();
        }
    }
    ...
    {
        $must_refresh = \Wappointment\WP\Helpers::getOption('appointments_must_refresh');
        // 0 - only check if site connected
        if (!empty($this->site_key) && (bool) $must_refresh === true && $must_refresh < \time()) {
            ...
            $appointments_update = \Wappointment\WP\Helpers::getOption('appointments_update');
            ...
            if ($this->hasPendingChanges($appointments, $appointments_update)) {
                ...
                if (!empty($requires_update)) {
                    $retrieved_appointments = \Wappointment\Models\Appointment::select('id', 'options')->whereIn('id', \array_keys($requires_update))->get();
                ...
                \Wappointment\WP\Helpers::setOption('appointments_update', $appointments);
            }
            $this->clearMustRefresh();
        }
    }
    ...
    {
        \Wappointment\WP\Helpers::setOption('appointments_must_refresh', false);
    }
    public function setMustRefresh()
    {
        \Wappointment\WP\Helpers::setOption('appointments_must_refresh', \time() + 60);
    }
    ...
    public function disconnect()
    {
        ...
        if ($result) {
            //disconnect
            if ($this->isLegacy) {
                \Wappointment\Services\Settings::saveStaff('dotcom', false);
            } else {
    ...
    {
        $tz = $this->isLegacy ? \Wappointment\Services\Settings::getStaff('timezone', $this->staff_id) : $this->staff->getTimezone();
    ...
    {
        $dotcom = \Wappointment\Services\Settings::getStaff('dotcom', $this->staff_id);
    ...
    protected function confirmAccountKey($result, $account_key)
    {
        if (empty($this->getSiteKey())) {
            \Wappointment\WP\Helpers::setOption('site_key', $result->sitekey);
        }
        $data = ['services' => $result->social, 'account_key' => $account_key];
        if ($this->isLegacy) {
            \Wappointment\Services\Settings::saveStaff('dotcom', $data);
        } else {
            ...
            $options = $this->staff->options;
            \Wappointment\Services\Flag::save('dotcomSet', true);
            $options['dotcom'] = $data;
    ...
    {
        return \Wappointment\Managers\Central::get('CalendarModel')::where('account_key', $account_key)->where('id', '!=', $this->staff_id)->first();
    }
}
