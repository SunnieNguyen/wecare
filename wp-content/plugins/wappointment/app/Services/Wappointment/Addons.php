...
class Addons extends \Wappointment\Services\Wappointment\API
{
            ...
            foreach ($data->addons as &$package) {
                foreach ($solutions as $solution) {
                    if ($solution->package_key == $package->key || $this->isAPlugin($package) && $this->getPluginDetails($package)->id == $solution->id) {
                        $expires_at_carbon = new \Wappointment\ClassConnect\Carbon($solution->expires_at);
                        $package->expires_at = $expires_at_carbon->format('d/m/Y');
                        $carbonnow = \Wappointment\ClassConnect\Carbon::now();
    ...
    {
        return !empty(\Wappointment\WP\Helpers::getOption('site_details')) ? \json_decode(\Wappointment\WP\Helpers::getOption('site_details')) : [];
    }
}
