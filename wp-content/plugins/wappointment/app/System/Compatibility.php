...
class Compatibility
{
    public static function getRestUrl()
    {
        if (!empty($_GET['wappo_ugly_permalinks'])) {
            \Wappointment\Services\Settings::save('force_ugly_permalinks', true);
        }
        return esc_url_raw(static::requiresUgly() ? static::getUglyRestUrl() : rest_url());
    }
    protected static function requiresUgly()
    {
        return \Wappointment\Services\Settings::get('force_ugly_permalinks') || empty(get_option('permalink_structure'));
    }
    /**
