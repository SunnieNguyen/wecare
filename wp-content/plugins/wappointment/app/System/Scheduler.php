...
class Scheduler
{
    ...
    public static function syncCalendar()
    {
        if (!\Wappointment\Services\VersionDB::atLeast(\Wappointment\Services\VersionDB::CAN_CREATE_SERVICES)) {
            ...
            foreach (\Wappointment\Services\Calendars::all(false, true) as $calendar) {
                (new \Wappointment\Services\Availability($calendar))->syncAndRegen();
            }
    ...
    public static function syncCalendarLegacy()
    {
        ...
        $calendar_urls = \Wappointment\WP\Helpers::getStaffOption('cal_urls', $staff_id);
        $hasChanged = false;
    ...
    {
        \Wappointment\Services\Flag::save('cronLastRun', \time());
    ...
    public static function registerCleanPending()
    {
        ...
        if (!\Wappointment\Services\Flag::get('CleanPending')) {
            \Wappointment\Jobs\CleanPendingPaymentAppointment::registerJob();
            \Wappointment\Services\Flag::save('CleanPending', true);
        }
    }
    /**
    ...
    public static function dailyProcess()
    {
        try {
            if (!\Wappointment\Services\VersionDB::atLeast(\Wappointment\Services\VersionDB::CAN_CREATE_SERVICES)) {
                ...
                foreach (\Wappointment\Services\Calendars::all(false, true) as $calendar) {
                    (new \Wappointment\Services\Availability($calendar))->regenerate();
                }
                (new \Wappointment\Repositories\CalendarsBack())->refresh();
            }
            (new \Wappointment\Services\Recurrent())->generate();
