...
class InitPreinstall
{
    ...
    public function customPluginRowMeta($links, $file)
    {
        if (\strpos($file, $this->plugin_file) !== false) {
            $buttonInit = '<a href="' . \Wappointment\WP\Helpers::adminUrl('wappointment_calendar') . '" class="button button-primary button-large" >' . __('Setup', 'wappointment') . '</a>';
    ...
    {
        $links[] = '<a href="' . esc_url(\Wappointment\WP\Helpers::adminUrl('wappointment_calendar')) . '" >' . __('Start Setup', 'wappointment') . '</a>';
        return $links;
    }
}
