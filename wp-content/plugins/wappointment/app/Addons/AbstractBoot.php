...
abstract class AbstractBoot implements \Wappointment\Addons\Boot
{
    ...
    {
        if (!empty(static::$addon_db_version_required) && static::isValid() && static::isInstalled() && \Wappointment\Services\CurrentUser::isAdmin()) {
    ...
    public static function getAddonsVersions($reset = false)
    {
        ...
        if ($addons_versions === false) {
            $addons_versions = \Wappointment\WP\Helpers::getOption('addons_db_version', []);
        }
        if ($reset !== false) {
    ...
    public static function setAddonDbVersion()
    {
        ...
        return \Wappointment\WP\Helpers::setOption('addons_db_version', $addons_versions);
    }
    ...
    {
        if (\Wappointment\Services\CurrentUser::isAdmin() && static::requiresDbUpdate()) {
    ...
    public static function licenceExpired()
    {
        ...
        $expires_at_carbon = new \Wappointment\ClassConnect\Carbon($licence->expires_at);
    ...
    public static function versionUpdateWarning($plugin)
    {
        if (static::licenceExpired() && !static::hasAccessToNewVersion($plugin['new_version'])) {
            ...
            ?>
                        <a href="<?php 
            echo WAPPOINTMENT_SITE . "/renew/site_" . \Wappointment\WP\Helpers::getOption('site_key');
            ?>" target="_blank"><?php 
...
<?php 
    ...
    protected static function getLicenceDetail()
    {
        ...
        if ($siteDetails === false) {
            $siteDetails = \Wappointment\Services\Wappointment\Addons::licensedSolutions();
        }
    ...
    {
        return \Wappointment\Helpers\Get::list('addons')[static::$addon_key]['min'];
    }
